...
from tuiuiu.tests.testapp.models import CustomImage, CustomImageFilePath
from tuiuiu.tests.utils import TuiuiuTestUtils
from tuiuiu.tuiuiuimages import get_image_model, get_image_model_string
from tuiuiu.tuiuiuimages.fields import TuiuiuImageField
from tuiuiu.tuiuiuimages.formats import Format, get_image_format, register_image_format
from tuiuiu.tuiuiuimages.forms import get_image_form
from tuiuiu.tuiuiuimages.models import Image as TuiuiuImage
from tuiuiu.tuiuiuimages.rect import Rect, Vector
from tuiuiu.tuiuiuimages.views.serve import ServeView, generate_signature, verify_signature
...
class TestImageTag(TestCase):
    ...
    def render_image_tag(self, image, filter_spec):
        temp = template.Template('{% load tuiuiuimages_tags %}{% image image_obj ' + filter_spec + '%}')
    ...
    def render_image_tag_as(self, image, filter_spec):
        temp = template.Template(
            '{% load tuiuiuimages_tags %}{% image image_obj ' + filter_spec +
    ...
    def render_image_tag_with_extra_attributes(self, image, title):
        temp = template.Template(
            '{% load tuiuiuimages_tags %}{% image image_obj width-400 \
    ...
    def render_image_tag_with_filters(self, image):
        temp = template.Template(
            '{% load tuiuiuimages_tags %}{% image image_primary|default:image_alternate width-400 %}'
        )
    ...
    def test_context_may_only_contain_one_argument(self):
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template(
                '{% load tuiuiuimages_tags %}{% image image_obj fill-200x200'
...
class TestFrontendServeView(TestCase):
    ...
    def test_get(self):
        ...
        # Get the image
        response = self.client.get(reverse('tuiuiuimages_serve', args=(signature, self.image.id, 'fill-800x600')))
    ...
    def test_get_with_extra_component(self):
        ...
        # Get the image
        response = self.client.get(reverse('tuiuiuimages_serve', args=(signature, self.image.id, 'fill-800x600')) + 'test.png')
    ...
    def test_get_with_too_many_extra_components(self):
        ...
        # Get the image
        response = self.client.get(reverse('tuiuiuimages_serve', args=(signature, self.image.id, 'fill-800x600')) + 'test/test.png')
    ...
    def test_get_with_serve_action(self):
        signature = generate_signature(self.image.id, 'fill-800x600')
        response = self.client.get(reverse('tuiuiuimages_serve_action_serve', args=(signature, self.image.id, 'fill-800x600')))
    ...
    def test_get_with_redirect_action(self):
        signature = generate_signature(self.image.id, 'fill-800x600')
        response = self.client.get(reverse('tuiuiuimages_serve_action_redirect', args=(signature, self.image.id, 'fill-800x600')))
    ...
    def test_get_with_custom_key(self):
        ...
        # Get the image
        response = self.client.get(reverse('tuiuiuimages_serve_custom_key', args=(signature, self.image.id, 'fill-800x600')) + 'test.png')
    ...
    def test_get_invalid_filter_spec(self):
        ...
        # Get the image
        response = self.client.get(reverse('tuiuiuimages_serve', args=(signature, self.image.id, 'bad-filter-spec')))
...
class TestFrontendSendfileView(TestCase):
    ...
    def test_sendfile_nobackend(self):
        signature = generate_signature(self.image.id, 'fill-800x600')
        response = self.client.get(reverse('tuiuiuimages_sendfile',
    ...
    def test_sendfile_dummy_backend(self):
        signature = generate_signature(self.image.id, 'fill-800x600')
        response = self.client.get(reverse('tuiuiuimages_sendfile_dummy',
                                           args=(signature, self.image.id,
                                                 'fill-800x600')))
...
class TestGetImageForm(TestCase, TuiuiuTestUtils):
...
class TestGetImageModel(TuiuiuTestUtils, TestCase):
    @override_settings(TUIUIUIMAGES_IMAGE_MODEL='tests.CustomImage')
    ...
    def test_standard_get_image_model_string(self):
        """Test get_image_model_STRING with no TUIUIUIMAGES_IMAGE_MODEL"""
        del settings.TUIUIUIMAGES_IMAGE_MODEL
        self.assertEqual(get_image_model_string(), 'tuiuiuimages.Image')

    @override_settings(TUIUIUIMAGES_IMAGE_MODEL='tests.UnknownModel')
    def test_unknown_get_image_model(self):
        """Test get_image_model with an unknown model"""
        with self.assertRaises(ImproperlyConfigured):
            get_image_model()

    @override_settings(TUIUIUIMAGES_IMAGE_MODEL='invalid-string')
