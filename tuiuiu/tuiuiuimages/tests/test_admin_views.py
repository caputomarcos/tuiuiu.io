...
from tuiuiu.tuiuiucore.models import Collection, GroupCollectionPermission
...
class TestImageIndexView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiuimages:index'), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/images/index.html')
...
class TestImageAddView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiuimages:add'), params)

    def post(self, post_data={}):
        return self.client.post(reverse('tuiuiuimages:add'), post_data)
    ...
    @override_settings(DEFAULT_FILE_STORAGE='tuiuiu.tests.dummy_external_storage.DummyExternalStorage')
    ...
    @override_settings(TUIUIUIMAGES_MAX_UPLOAD_SIZE=1)
...
class TestImageAddViewWithLimitedCollectionPermissions(TestCase, TuiuiuTestUtils):
    def setUp(self):
...
class TestImageEditView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(reverse('tuiuiuimages:edit', args=(self.image.id,)), params)

    def post(self, post_data={}):
        return self.client.post(reverse('tuiuiuimages:edit', args=(self.image.id,)), post_data)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/images/edit.html')

        # Ensure the form supports file uploads
        self.assertContains(response, 'enctype="multipart/form-data"')

    @override_settings(TUIUIU_USAGE_COUNT_ENABLED=True)
...
class TestImageDeleteView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(reverse('tuiuiuimages:delete', args=(self.image.id,)), params)

    def post(self, post_data={}):
        return self.client.post(reverse('tuiuiuimages:delete', args=(self.image.id,)), post_data)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/images/confirm_delete.html')
...
class TestImageChooserView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.user = self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiuimages:chooser'), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/chooser/chooser.html')
        self.assertTemplateUsed(response, 'tuiuiuimages/chooser/chooser.js')
...
class TestImageChooserChosenView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(reverse('tuiuiuimages:image_chosen', args=(self.image.id,)), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/chooser/image_chosen.js')


class TestImageChooserSelectFormatView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(reverse('tuiuiuimages:chooser_select_format', args=(self.image.id,)), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/chooser/select_format.html')
        self.assertTemplateUsed(response, 'tuiuiuimages/chooser/select_format.js')
...
class TestImageChooserUploadView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiuimages:chooser_upload'), params)
    ...
    def test_upload(self):
        response = self.client.post(reverse('tuiuiuimages:chooser_upload'), {
            'title': "Test image",
            'file': SimpleUploadedFile('test.png', get_test_image_file().file.getvalue()),
        })
...
class TestImageChooserUploadViewWithLimitedPermissions(TestCase, TuiuiuTestUtils):
    def setUp(self):
        add_image_permission = Permission.objects.get(
            content_type__app_label='tuiuiuimages', codename='add_image'
        )
    ...
    def test_get(self):
        response = self.client.get(reverse('tuiuiuimages:chooser_upload'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/chooser/chooser.html')
    ...
    def test_get_chooser(self):
        response = self.client.get(reverse('tuiuiuimages:chooser'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/chooser/chooser.html')
...
class TestMultipleImageUploader(TestCase, TuiuiuTestUtils):
    """
    This tests the multiple image upload views located in tuiuiuimages/views/multiple.py
    """
    def setUp(self):
        self.login()
    ...
    @override_settings(TUIUIUIMAGES_MAX_UPLOAD_SIZE=1000)
    ...
    def test_add_post(self):
        ...
        """
        response = self.client.post(reverse('tuiuiuimages:add_multiple'), {
        ...
        self.assertTemplateUsed(response, 'tuiuiuimages/multiple/edit_form.html')
    ...
    def test_add_post_nofile(self):
        ...
        """
        response = self.client.post(reverse('tuiuiuimages:add_multiple'), {}, HTTP_X_REQUESTED_WITH='XMLHttpRequest')
    ...
    def test_edit_get(self):
        ...
        """
        # Send request
        response = self.client.get(reverse('tuiuiuimages:edit_multiple', args=(self.image.id, )))
    ...
    def test_edit_post(self):
        ...
        """
        # Send request
        response = self.client.post(reverse('tuiuiuimages:edit_multiple', args=(self.image.id, )), {
    ...
    def test_delete_get(self):
        ...
        """
        # Send request
        response = self.client.get(reverse('tuiuiuimages:delete_multiple', args=(self.image.id, )))
    ...
    def test_delete_post(self):
        ...
        response = self.client.post(reverse(
            'tuiuiuimages:delete_multiple', args=(self.image.id, )
        ), HTTP_X_REQUESTED_WITH='XMLHttpRequest')
    ...
    def test_delete_post_noajax(self):
        ...
        """
        # Send request
        response = self.client.post(reverse('tuiuiuimages:delete_multiple', args=(self.image.id, )))

        # Check response
        self.assertEqual(response.status_code, 400)


class TestURLGeneratorView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Create an image for running tests on
        self.image = Image.objects.create(
            title="Test image",
    ...
    def test_get(self):
        ...
        """
        # Get
        response = self.client.get(reverse('tuiuiuimages:url_generator', args=(self.image.id, )))

        # Check response
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/images/url_generator.html')
...
class TestGenerateURLView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Create an image for running tests on
        self.image = Image.objects.create(
            title="Test image",
    ...
    def test_get(self):
        ...
        """
        # Get
        response = self.client.get(reverse('tuiuiuimages:generate_url', args=(self.image.id, 'fill-800x600')))
        ...
        expected_preview_url = reverse('tuiuiuimages:preview', args=(self.image.id, 'fill-800x600'))
    ...
    def test_get_bad_image(self):
        ...
        """
        # Get
        response = self.client.get(reverse('tuiuiuimages:generate_url', args=(self.image.id + 1, 'fill-800x600')))
    ...
    def test_get_bad_filter_spec(self):
        ...
        """
        # Get
        response = self.client.get(reverse('tuiuiuimages:generate_url', args=(self.image.id, 'bad-filter-spec')))
...
class TestPreviewView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Create an image for running tests on
        self.image = Image.objects.create(
            title="Test image",
    ...
    def test_get(self):
        ...
        """
        # Get the image
        response = self.client.get(reverse('tuiuiuimages:preview', args=(self.image.id, 'fill-800x600')))
    ...
    def test_get_invalid_filter_spec(self):
        ...
        """
        # Get the image
        response = self.client.get(reverse('tuiuiuimages:preview', args=(self.image.id, 'bad-filter-spec')))

        # Check response
        self.assertEqual(response.status_code, 400)


class TestEditOnlyPermissions(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Create an image to edit
        self.image = Image.objects.create(
            title="Test image",
        ...
        change_permission = Permission.objects.get(content_type__app_label='tuiuiuimages', codename='change_image')
        admin_permission = Permission.objects.get(content_type__app_label='tuiuiuadmin', codename='access_admin')
    ...
    def test_get_index(self):
        response = self.client.get(reverse('tuiuiuimages:index'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/images/index.html')
    ...
    def test_search(self):
        response = self.client.get(reverse('tuiuiuimages:index'), {'q': "Hello"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['query_string'], "Hello")

    def test_get_add(self):
        response = self.client.get(reverse('tuiuiuimages:add'))
        # permission should be denied
        self.assertRedirects(response, reverse('tuiuiuadmin_home'))

    def test_get_edit(self):
        response = self.client.get(reverse('tuiuiuimages:edit', args=(self.image.id,)))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/images/edit.html')

    def test_get_delete(self):
        response = self.client.get(reverse('tuiuiuimages:delete', args=(self.image.id,)))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuimages/images/confirm_delete.html')
...
class TestImageAddMultipleView(TestCase, TuiuiuTestUtils):
    ...
    def test_as_ordinary_editor(self):
        ...
        add_permission = Permission.objects.get(content_type__app_label='tuiuiuimages', codename='add_image')
        admin_permission = Permission.objects.get(content_type__app_label='tuiuiuadmin', codename='access_admin')
