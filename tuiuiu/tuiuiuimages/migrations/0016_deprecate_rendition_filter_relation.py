...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('tuiuiuimages', '0015_fill_filter_spec_field'),
    ]
    ...
    operations = [
        ...
        # New step introduced in Tuiuiu 1.8.1:
        #
        # Reduce max_length of rendition.focal_point_key to 16, from the previous value of 255
        # which existed on Tuiuiu <= 1.8. MySQL has a limit of 767 (on InnoDB) or 1000 (on MyISAM)
        ...
        #   failed on tuiuiuimages/0001_initial can now run (see #2925 / #2953);
        # * Here, so that previously-working Tuiuiu <=1.7 installations that failed on the
        #   AlterUniqueTogether below when upgrading to 1.8 can now succeed;
        # * In the newly-added migration tuiuiuimages/0017, so that existing Tuiuiu 1.8 installations
