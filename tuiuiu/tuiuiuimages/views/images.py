...
from tuiuiu.tuiuiuimages.exceptions import InvalidFilterSpecError
from tuiuiu.tuiuiuimages.forms import URLGeneratorForm, get_image_form
from tuiuiu.tuiuiuimages.models import Filter
...
def index(request):
    ...
    # Create response
    if request.is_ajax():
        return render(request, 'tuiuiuimages/images/results.html', {
            'images': images,
            'query_string': query_string,
            'is_searching': bool(query_string),
        })
    else:
        return render(request, 'tuiuiuimages/images/index.html', {
            'images': images,
            'query_string': query_string,
            'is_searching': bool(query_string),
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, _("Image '{0}' updated.").format(image.title), buttons=[
                messages.button(reverse('tuiuiuimages:edit', args=(image.id,)), _('Edit again'))
            ])
    ...
    if image.is_stored_locally():
        ...
        if not os.path.isfile(image.file.path):
            messages.error(request, _(
                ...
                messages.button(reverse('tuiuiuimages:delete', args=(image.id,)), _('Delete'))
            ])

    return render(request, "tuiuiuimages/images/edit.html", {
        'image': image,
        'form': form,
...
def generate_url(request, image_id, filter_spec):
    ...
    url = reverse('tuiuiuimages_serve', args=(signature, image_id, filter_spec))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, _("Image '{0}' added.").format(image.title), buttons=[
                messages.button(reverse('tuiuiuimages:edit', args=(image.id,)), _('Edit'))
            ])
            return redirect('tuiuiuimages:index')
        else:
