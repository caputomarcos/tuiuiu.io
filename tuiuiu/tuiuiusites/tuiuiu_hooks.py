...
def register_viewset():
    return SiteViewSet('tuiuiusites', url_prefix='sites')
...
def register_sites_menu_item():
    return SitesMenuItem(_('Sites'), urlresolvers.reverse('tuiuiusites:index'),
                         classnames='icon icon-site', order=602)


@hooks.register('register_permissions')
def register_permissions():
    return Permission.objects.filter(content_type__app_label='tuiuiucore',
