...
def index(request):
    ...
    # Create response
    if request.is_ajax():
        return render(request, 'tuiuiudocs/documents/results.html', {
            'ordering': ordering,
            'documents': documents,
        ...
        return render(request, 'tuiuiudocs/documents/index.html', {
            'ordering': ordering,
            'documents': documents,
            'query_string': query_string,
            'is_searching': bool(query_string),
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, _("Document '{0}' added.").format(doc.title), buttons=[
                messages.button(reverse('tuiuiudocs:edit', args=(doc.id,)), _('Edit'))
            ])
...
def edit(request, document_id):
    ...
    if not filesize:
        messages.error(
            ...
            buttons=[messages.button(reverse('tuiuiudocs:delete', args=(doc.id,)), _('Delete'))]
        )

    return render(request, "tuiuiudocs/documents/edit.html", {
        'document': doc,
        'filesize': filesize,
        'form': form,
