...
from tuiuiu.tuiuiuadmin.utils import get_object_usage
from tuiuiu.tuiuiucore.models import CollectionMember
...
def get_document_model():
    ...
    try:
        app_label, model_name = settings.TUIUIUDOCS_DOCUMENT_MODEL.split('.')
    except AttributeError:
        return Document
    except ValueError:
        raise ImproperlyConfigured("TUIUIUDOCS_DOCUMENT_MODEL must be of the form 'app_label.model_name'")

    document_model = apps.get_model(app_label, model_name)
    if document_model is None:
        raise ImproperlyConfigured(
            "TUIUIUDOCS_DOCUMENT_MODEL refers to model '%s' that has not been installed" %
            settings.TUIUIUDOCS_DOCUMENT_MODEL
        )
