...
class TestCollectionPrivacyDocument(TestCase):
    ...
    def get_document(self, collection):
        ...
        url = reverse('tuiuiudocs_serve', args=(secret_document.id, secret_document.filename))
    ...
    def test_anonymous_user_must_authenticate(self):
        ...
        doc_url = reverse('tuiuiudocs_serve', args=(secret_document.id, secret_document.filename))
        response = self.client.get(doc_url)
        self.assertEqual(response.templates[0].name, 'tuiuiudocs/password_required.html')

        submit_url = reverse('tuiuiudocs_authenticate_with_password', args=[self.view_restriction.id])
