...
class TestServeView(TestCase):
    ...
    def get(self):
        return self.client.get(reverse('tuiuiudocs_serve', args=(self.document.id, self.document.filename)))
    ...
    def test_with_nonexistent_document(self):
        response = self.client.get(reverse('tuiuiudocs_serve', args=(1000, 'blahblahblah', )))
        self.assertEqual(response.status_code, 404)

    def test_with_incorrect_filename(self):
        response = self.client.get(reverse('tuiuiudocs_serve', args=(self.document.id, 'incorrectfilename')))
        self.assertEqual(response.status_code, 404)

    def clear_sendfile_cache(self):
        from tuiuiu.utils.sendfile import _get_sendfile
        _get_sendfile.clear()
...
class TestServeWithUnicodeFilename(TestCase):
    ...
    def test_response_code(self):
        response = self.client.get(reverse('tuiuiudocs_serve', args=(self.document.id, self.filename)))
        self.assertEqual(response.status_code, 200)
