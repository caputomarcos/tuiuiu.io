...
class TestDocumentListing(TestCase):
    ...
    def get_response(self, **params):
        return self.client.get(reverse('tuiuiuapi_v2:documents:listing'), params)
    ...
    def test_search_when_filtering_by_tag_gives_error(self):
        response = self.get_response(search='james', tags='tuiuiu')
        content = json.loads(response.content.decode('UTF-8'))
...
class TestDocumentDetail(TestCase):
    ...
    def get_response(self, image_id, **params):
        return self.client.get(reverse('tuiuiuapi_v2:documents:detail', args=(image_id, )), params)
    ...
    def test_basic(self):
        ...
        self.assertEqual(content['meta']['type'], 'tuiuiudocs.Document')
        ...
        self.assertEqual(content['meta']['download_url'], 'http://localhost/documents/1/tuiuiu_by_markyharky.jpg')

        # Check the title field
        self.assertIn('title', content)
        self.assertEqual(content['title'], "Tuiuiu by mark Harkin")
    ...
    @override_settings(TUIUIUAPI_BASE_URL='http://api.example.com/')
    def test_download_url_with_custom_base_url(self):
        response = self.get_response(1)
        content = json.loads(response.content.decode('UTF-8'))

        self.assertIn('download_url', content['meta'])
        self.assertEqual(content['meta']['download_url'], 'http://api.example.com/documents/1/tuiuiu_by_markyharky.jpg')
