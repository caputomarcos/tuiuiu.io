...
from tuiuiu.api.v2 import signal_handlers
from tuiuiu.tests.demosite import models
from tuiuiu.tests.testapp.models import StreamPage
...
class TestPageListing(TestCase):
    ...
    def get_response(self, **params):
        return self.client.get(reverse('tuiuiuapi_v2:pages:listing'), params)
    ...
    def test_fields_foreign_key(self):
        ...
        for page in content['items']:
            ...
            if feed_image is not None:
                ...
                self.assertEqual(feed_image['meta']['type'], 'tuiuiuimages.Image')
    ...
    def test_filtering_tags(self):
        response = self.get_response(type='demosite.BlogEntryPage', tags='tuiuiu')
        content = json.loads(response.content.decode('UTF-8'))
    ...
    def test_filtering_multiple_tags(self):
        response = self.get_response(type='demosite.BlogEntryPage', tags='tuiuiu,bird')
        content = json.loads(response.content.decode('UTF-8'))
    ...
    @override_settings(TUIUIUAPI_LIMIT_MAX=10)
    ...
    @override_settings(TUIUIUAPI_LIMIT_MAX=2)
    def test_limit_default_changes_with_max(self):
        # The default limit is 20. If TUIUIUAPI_LIMIT_MAX is less than that,
    ...
    @override_settings(TUIUIUAPI_SEARCH_ENABLED=False)
    ...
    def test_search_when_filtering_by_tag_gives_error(self):
        response = self.get_response(type='demosite.BlogEntryPage', search='blog', tags='tuiuiu')
        content = json.loads(response.content.decode('UTF-8'))
...
class TestPageDetail(TestCase):
    ...
    def get_response(self, page_id, **params):
        return self.client.get(reverse('tuiuiuapi_v2:pages:detail', args=(page_id, )), params)
    ...
    def test_basic(self):
        ...
        self.assertEqual(content['tags'], ['bird', 'tuiuiu'])
        ...
        self.assertEqual(content['feed_image']['meta']['type'], 'tuiuiuimages.Image')
...
class TestPageDetailWithStreamField(TestCase):
    ...
    def test_can_fetch_streamfield_content(self):
        ...
        response_url = reverse('tuiuiuapi_v2:pages:detail', args=(stream_page.id, ))
    ...
    def test_image_block_with_custom_get_api_representation(self):
        ...
        response_url = '{}?extended=1'.format(
            reverse('tuiuiuapi_v2:pages:detail', args=(stream_page.id, ))
        )
        response = self.client.get(response_url)
        content = json.loads(response.content.decode('utf-8'))
...
@override_settings(
    FRONTENDCACHE={
        'varnish': {
            'BACKEND': 'tuiuiu.contrib.frontendcache.backends.HTTPBackend',
            'LOCATION': 'http://localhost:8000',
        },
    },
    TUIUIUAPI_BASE_URL='http://api.example.com',
)
@mock.patch('tuiuiu.contrib.frontendcache.backends.HTTPBackend.purge')
