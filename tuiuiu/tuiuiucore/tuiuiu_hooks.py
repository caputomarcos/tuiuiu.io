...
def require_tuiuiu_login(next):
    login_url = getattr(settings, 'TUIUIU_FRONTEND_LOGIN_URL', reverse('tuiuiucore_login'))
    ...
    for restriction in page.get_view_restrictions():
        if not restriction.accept_request(request):
            if restriction.restriction_type == PageViewRestriction.PASSWORD:
                ...
                action_url = reverse('tuiuiucore_authenticate_with_password', args=[restriction.id, page.id])
