...
from tuiuiu.utils.compat import user_is_authenticated
from tuiuiu.utils.deprecation import RemovedInTuiuiu113Warning
from tuiuiu.tuiuiucore.query import PageQuerySet, TreeQuerySet
from tuiuiu.tuiuiucore.signals import page_published, page_unpublished
from tuiuiu.tuiuiucore.sites import get_site_for_hostname
from tuiuiu.tuiuiucore.url_routing import RouteResult
from tuiuiu.tuiuiucore.utils import (
    TUIUIU_APPEND_SLASH, accepts_kwarg, camelcase_to_underscore, resolve_model_string)
...
class Site(models.Model):
    ...
    def get_site_root_paths():
        ...
        """
        result = cache.get('tuiuiu_site_root_paths')
        ...
        if result is None:
            ...
            cache.set('tuiuiu_site_root_paths', result, 3600)
...
class Page(six.with_metaclass(PageBase, AbstractPage, index.Indexed, ClusterableModel)):
    ...
    # Do not allow plain Page instances to be created through the Tuiuiu admin
    is_creatable = False

    # Define these attributes early to avoid masking errors. (Issue #3078)
    # The canonical definition is in tuiuiuadmin.edit_handlers.
    content_panels = []
    promote_panels = []
    settings_panels = []
    ...
    def save(self, *args, **kwargs):
        ...
        # Check if this is a root page of any sites and clear the 'tuiuiu_site_root_paths' key if so
    ...
    def check(cls, **kwargs):
                ...
                if field.rel.on_delete == models.CASCADE:
                    errors.append(
                        checks.Warning(
                            ...
                            hint="Set on_delete=models.SET_NULL and make sure the field is nullable or set on_delete=models.PROTECT. Tuiuiu does not allow simple database CASCADE because it will corrupt its tree storage.",
                            obj=field,
                            id='tuiuiucore.W001',
                        )
                    )
        ...
        if not isinstance(cls.objects, PageManager):
            errors.append(
                checks.Error(
                    ...
                    hint="Ensure that custom Page managers inherit from tuiuiu.tuiuiucore.models.PageManager",
                    obj=cls,
                    id='tuiuiucore.E002',
                )
            )
        ...
        if not accepts_kwarg(cls.relative_url, 'request'):
            warnings.warn(
                ...
                "See http://docs.tuiuiu.io/en/v1.11/reference/pages/model_reference.html#tuiuiu.tuiuiucore.models.Page.relative_url" % cls,
                RemovedInTuiuiu113Warning)

        if not accepts_kwarg(cls.get_url_parts, 'request'):
            warnings.warn(
                "%s.get_url_parts should accept a 'request' keyword argument. "
                "See http://docs.tuiuiu.io/en/v1.11/reference/pages/model_reference.html#tuiuiu.tuiuiucore.models.Page.get_url_parts" % cls,
                RemovedInTuiuiu113Warning)
    ...
    def _get_site_root_paths(self, request=None):
        ...
        try:
            return cache_object._tuiuiu_cached_site_root_paths
        except AttributeError:
            cache_object._tuiuiu_cached_site_root_paths = Site.get_site_root_paths()
            return cache_object._tuiuiu_cached_site_root_paths
    ...
    def _safe_get_url_parts(self, request=None):
        ...
        """
        # RemovedInTuiuiu113Warning - this accepts_kwarg test can be removed when we drop support
    ...
    def get_url_parts(self, request=None):
        ...
        for (site_id, root_path, root_url) in self._get_site_root_paths(request):
            if self.url_path.startswith(root_path):
                page_path = reverse('tuiuiu_serve', args=(self.url_path[len(root_path):],))

                # Remove the trailing slash from the URL reverse generates if
                # TUIUIU_APPEND_SLASH is False and we're not trying to serve
                # the root path
                if not TUIUIU_APPEND_SLASH and page_path != '/':
    ...
    def get_indexed_instance(self):
        # This is accessed on save by the tuiuiusearch signal handler, and in edge
    ...
    def serve_preview(self, request, mode_name):
        ...
        Tuiuiu will allow the user to specify one of those modes when previewing, and
        ...
        the tuiuiu user bar to be displayed. This request will always be a GET.
        """
    ...
    password_required_template = getattr(settings, 'PASSWORD_REQUIRED_TEMPLATE', 'tuiuiucore/password_required.html')
