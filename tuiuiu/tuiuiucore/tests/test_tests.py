...
from tuiuiu.tests.utils import TuiuiuPageTests, TuiuiuTestUtils
from tuiuiu.tuiuiucore.models import PAGE_MODEL_CLASSES, Page, Site


class TestAssertTagInHTML(TuiuiuTestUtils, TestCase):
...
class TestTuiuiuPageTests(TuiuiuPageTests):
    def setUp(self):
        super(TestTuiuiuPageTests, self).setUp()
        site = Site.objects.get(is_default_site=True)
        self.root = site.root_page.specific

    def assertRaisesRegex(self, *args, **kwargs):
        if six.PY3:
            return super(TestTuiuiuPageTests, self).assertRaisesRegex(*args, **kwargs)
        else:
