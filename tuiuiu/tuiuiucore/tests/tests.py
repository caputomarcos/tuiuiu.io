...
from tuiuiu.tuiuiucore.templatetags.tuiuiucore_tags import richtext
...
class TestPageUrlTags(TestCase):
    ...
    def test_pageurl_without_request_in_context(self):
        ...
        tpl = template.Template('''{% load tuiuiucore_tags %}<a href="{% pageurl page %}">{{ page.title }}</a>''')
    ...
    def test_slugurl_without_request_in_context(self):
        tpl = template.Template('''{% load tuiuiucore_tags %}<a href="{% slugurl 'events' %}">Events</a>''')
...
class TestSiteRootPathsCache(TestCase):
    ...
    def test_cache(self):
        ...
        self.assertEqual(cache.get('tuiuiu_site_root_paths'), [(1, '/home/', 'http://localhost')])
    ...
    def test_cache_clears_when_site_saved(self):
        ...
        self.assertTrue(cache.get('tuiuiu_site_root_paths'))

        # Save the site
        Site.objects.get(is_default_site=True).save()

        # Check that the cache has been cleared
        self.assertFalse(cache.get('tuiuiu_site_root_paths'))
    ...
    def test_cache_clears_when_site_root_slug_changes(self):
        ...
        Fix: d6cce69a397d08d5ee81a8cbc1977ab2c9db2682
        Discussion: https://github.com/tuiuiu/tuiuiu/issues/157
        """
        # Get homepage
        homepage = Page.objects.get(url_path='/home/')
...
class TestResolveModelString(TestCase):
    ...
    def test_resolve_from_string_with_default_app(self):
        model = resolve_model_string('Page', default_app='tuiuiucore')

        self.assertEqual(model, Page)

    def test_resolve_from_string_with_different_default_app(self):
        model = resolve_model_string('tuiuiucore.Page', default_app='tuiuiuadmin')
    ...
    def test_resolve_from_string_invalid(self):
        self.assertRaises(ValueError, resolve_model_string, 'tuiuiu.tuiuiucore.Page')

    def test_resolve_from_string_with_incorrect_default_app(self):
        self.assertRaises(LookupError, resolve_model_string, 'Page', default_app='tuiuiuadmin')

    def test_resolve_from_string_with_unknown_model_string(self):
        self.assertRaises(LookupError, resolve_model_string, 'tuiuiuadmin.Page')
