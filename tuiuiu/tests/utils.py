...
from tuiuiu.tests.assert_logs import _AssertLogsContext


class TuiuiuTestUtils(object):
...
class TuiuiuPageTests(TuiuiuTestUtils, TestCase):
    """
    A set of asserts to help write tests for your own Tuiuiu site.
    """
    def setUp(self):
        super(TuiuiuPageTests, self).setUp()
        self.login()
    ...
    def assertCanCreate(self, parent, child_model, data, msg=None):
        ...
        Tuiuiu admin Page creation method.
        """
        ...
        if django.VERSION >= (1, 9):
            explore_url = reverse('tuiuiuadmin_explore', args=[parent.pk])
        else:
            explore_url = 'http://testserver' + reverse('tuiuiuadmin_explore', args=[parent.pk])
