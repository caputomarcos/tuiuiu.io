...
from tuiuiu.tuiuiuadmin.utils import send_mail
from tuiuiu.tuiuiucore.blocks import CharBlock, RichTextBlock
from tuiuiu.tuiuiucore.fields import RichTextField, StreamField
from tuiuiu.tuiuiucore.models import Orderable, Page, PageManager
from tuiuiu.tuiuiudocs.edit_handlers import DocumentChooserPanel
from tuiuiu.tuiuiuforms.models import AbstractEmailForm, AbstractFormField, AbstractFormSubmission
from tuiuiu.tuiuiuimages.blocks import ImageChooserBlock
from tuiuiu.tuiuiuimages.edit_handlers import ImageChooserPanel
from tuiuiu.tuiuiuimages.models import AbstractImage, AbstractRendition, Image
...
class PageWithOldStyleRouteMethod(Page):
    """
    Prior to Tuiuiu 0.4, the route() method on Page returned an HttpResponse
...
# Override the standard TuiuiuAdminPageForm to add validation on start/end dates
...
class AdvertPlacement(models.Model):
    page = ParentalKey('tuiuiucore.Page', related_name='advert_placements', on_delete=models.CASCADE)
...
], base_form_class=TuiuiuAdminPageForm)
...
class PageChooserModel(models.Model):
    page = models.ForeignKey('tuiuiucore.Page', help_text='help text', on_delete=models.CASCADE)
...
class UserProfile(models.Model):
    # Tuiuiu's schema must be able to coexist alongside a custom UserProfile model
