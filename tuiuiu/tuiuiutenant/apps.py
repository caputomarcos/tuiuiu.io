...
from tuiuiu.tuiuiutenant.storage import TenantStorageMixin
from tuiuiu.tuiuiutenant.utils import get_public_schema_name, get_tenant_model


class TuiuiuTenantConfig(AppConfig):
    name = 'tuiuiu.tuiuiutenant'
    label = 'tuiuiutenant'
    verbose_name = "Tuiuiu tenant"
...
def best_practice(app_configs, **kwargs):
    ...
    if 'tuiuiu.tuiuiutenant.routers.TenantSyncRouter' not in settings.DATABASE_ROUTERS:
        return [
    ...
    if INSTALLED_APPS.index('tuiuiu.tuiuiutenant') > django_index:
        errors.append(
            Warning("You should put 'tuiuiu.tuiuiutenant' before any django "
    ...
    if not isinstance(default_storage, TenantStorageMixin):
        errors.append(Warning(
            ...
            hint="Set settings.DEFAULT_FILE_STORAGE to "
                 "'tuiuiu.tuiuiutenant.storage.TenantFileSystemStorage'",
            id="tuiuiutenant.W003"
        ))
