...
from tuiuiu.tuiuiucore.models import GroupPagePermission, Page, PageRevision
from tuiuiu.tuiuiuusers.models import UserProfile

logger = logging.getLogger('tuiuiu.admin')

# Tuiuiu languages with >=90% coverage
# This list is manually maintained
TUIUIUADMIN_PROVIDED_LANGUAGES = [
...
def get_available_admin_languages():
    return getattr(settings, 'TUIUIUADMIN_PERMITTED_LANGUAGES', TUIUIUADMIN_PROVIDED_LANGUAGES)
...
def permission_required(permission_name):
    ...
    (The latter doesn't work anyway because Tuiuiu doesn't define LOGIN_URL...)
    """
    def test(user):
...
class PermissionPolicyChecker(object):
    ...
    returning the tuiuiuadmin 'permission denied' response if permission not granted
    """
...
def send_mail(subject, message, recipient_list, from_email=None, **kwargs):
    if not from_email:
        if hasattr(settings, 'TUIUIUADMIN_NOTIFICATION_FROM_EMAIL'):
            from_email = settings.TUIUIUADMIN_NOTIFICATION_FROM_EMAIL
...
def send_notification(page_revision_id, notification, excluded_user_id):
    ...
    # Get template
    template_subject = 'tuiuiuadmin/notifications/' + notification + '_subject.txt'
    template_text = 'tuiuiuadmin/notifications/' + notification + '.txt'
    template_html = 'tuiuiuadmin/notifications/' + notification + '.html'
    ...
    for recipient in email_recipients:
        try:
            ...
            with override(recipient.tuiuiu_userprofile.get_preferred_language()):
            ...
            kwargs = {}
            if getattr(settings, 'TUIUIUADMIN_NOTIFICATION_USE_HTML', False):
