...
from tuiuiu.tuiuiuadmin import messages, signals
from tuiuiu.tuiuiuadmin.forms import CopyForm, SearchForm
...
from tuiuiu.tuiuiucore.models import Page, PageRevision, UserPagePermissionsProxy
...
def index(request, parent_page_id=None):
    ...
    return render(request, 'tuiuiuadmin/pages/index.html', {
...
def add_subpage(request, parent_page_id):
    ...
    if len(page_types) == 1:
        ...
        return redirect('tuiuiuadmin_pages:add', app_label, model_name, parent_page.id)

    return render(request, 'tuiuiuadmin/pages/add_subpage.html', {
...
def content_type_use(request, content_type_app_name, content_type_model_name):
    ...
    return render(request, 'tuiuiuadmin/pages/content_type_use.html', {
        'pages': pages,
...
def create(request, content_type_app_name, content_type_model_name, parent_page_id):
            ...
            if is_publishing:
                if page.go_live_at and page.go_live_at > timezone.now():
                    messages.success(request, _("Page '{0}' created and scheduled for publishing.").format(page.get_admin_display_title()), buttons=[
                        messages.button(reverse('tuiuiuadmin_pages:edit', args=(page.id,)), _('Edit'))
                    ])
                else:
                ...
                messages.success(
                    ...
                    buttons=[
                        messages.button(
                            reverse('tuiuiuadmin_pages:view_draft', args=(page.id,)),
                            _('View draft'),
                            new_window=True
                        ),
                        messages.button(
                            reverse('tuiuiuadmin_pages:edit', args=(page.id,)),
                            _('Edit')
                        )
                    ]
                )
    ...
    return render(request, 'tuiuiuadmin/pages/create.html', {
        'content_type': content_type,
        'page_class': page_class,
        'parent_page': parent_page,
            ...
            if is_publishing:
                if page.go_live_at and page.go_live_at > timezone.now():
                    ...
                    messages.success(request, message, buttons=[
                        ...
                        messages.button(
                            reverse('tuiuiuadmin_pages:edit', args=(page_id,)),
                            _('Edit')
                        )
                    ])
                ...
                messages.success(request, message, buttons=[
                    messages.button(
                        reverse('tuiuiuadmin_pages:view_draft', args=(page_id,)),
                        _('View draft'),
                        new_window=True
                    ),
                    messages.button(
    ...
    if latest_revision and latest_revision.submitted_for_moderation:
        ...
        if page.live:
            buttons.append(messages.button(
                reverse('tuiuiuadmin_pages:revisions_compare', args=(page.id, 'live', latest_revision.id)),
                _('Compare with live version')
            ))

        messages.warning(request, _("This page is currently awaiting moderation"), buttons=buttons)

    return render(request, 'tuiuiuadmin/pages/edit.html', {
        'page': page,
        'content_type': content_type,
        'edit_handler': edit_handler,
...
def delete(request, page_id):
    ...
    return render(request, 'tuiuiuadmin/pages/confirm_delete.html', {
        'page': page,
...
class PreviewOnEdit(View):
    ...
    session_key_prefix = 'tuiuiu-preview-'
    ...
    def error_response(self, page):
        return render(self.request, 'tuiuiuadmin/pages/preview_error.html',
                      {'page': page})
...
def unpublish(request, page_id):
    ...
    return render(request, 'tuiuiuadmin/pages/confirm_unpublish.html', {
        'page': page,
        'next': next_url,
...
def move_choose_destination(request, page_to_move_id, viewed_page_id=None):
    ...
    return render(request, 'tuiuiuadmin/pages/move_choose_destination.html', {
...
def move_confirm(request, page_to_move_id, destination_id):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, _("Page '{0}' moved.").format(page_to_move.get_admin_display_title()), buttons=[
            messages.button(reverse('tuiuiuadmin_pages:edit', args=(page_to_move.id,)), _('Edit'))
        ])
...
def search(request):
    ...
    if request.is_ajax():
        return render(request, "tuiuiuadmin/pages/search_results.html", {
            'pages': pages,
            'query_string': q,
            'pagination_query_params': ('q=%s' % q) if q else ''
        })
    else:
        return render(request, "tuiuiuadmin/pages/search.html", {
            'search_form': form,
            'pages': pages,
            'query_string': q,
            'pagination_query_params': ('q=%s' % q) if q else ''
...
def approve_moderation(request, revision_id):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, _("Page '{0}' published.").format(revision.page.get_admin_display_title()), buttons=[
            ...
            messages.button(reverse('tuiuiuadmin_pages:edit', args=(revision.page.id,)), _('Edit'))
        ])
...
def preview_for_moderation(request, revision_id):
    ...
    # and request.revision_id will be picked up by the tuiuiu user bar
...
def revisions_index(request, page_id):
    ...
    return render(request, 'tuiuiuadmin/pages/revisions/index.html', {
        'page': page,
        'ordering': ordering,
        'pagination_query_params': "ordering=%s" % ordering,
        'revisions': revisions,
...
def revisions_revert(request, page_id, revision_id):
    ...
    user_avatar = render_to_string('tuiuiuadmin/shared/user_avatar.html', {'user': revision.user})
...
def revisions_compare(request, page_id, revision_id_a, revision_id_b):
    ...
    return render(request, 'tuiuiuadmin/pages/revisions/compare.html', {
        'page': page,
