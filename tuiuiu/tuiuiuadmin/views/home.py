...
from tuiuiu.tuiuiuadmin.site_summary import SiteSummaryPanel
...
class UpgradeNotificationPanel(object):
    ...
    def render(self):
        if self.request.user.is_superuser and getattr(settings, "TUIUIU_ENABLE_UPDATE_CHECK", True):
            return render_to_string('tuiuiuadmin/home/upgrade_notification.html', {}, request=self.request)
        else:
            return ""
...
class PagesForModerationPanel(object):
    ...
    def render(self):
        return render_to_string('tuiuiuadmin/home/pages_for_moderation.html', {
...
class RecentEditsPanel(object):
    ...
    def __init__(self, request):
        ...
        # Last n edited pages
        edit_count = getattr(settings, 'TUIUIUADMIN_RECENT_EDITS_LIMIT', 5)
        if connection.vendor == 'mysql':
            ...
            last_edits = PageRevision.objects.raw(
                """
                SELECT wp.* FROM
                    tuiuiucore_pagerevision wp JOIN (
                        SELECT max(created_at) AS max_created_at, page_id FROM
                            tuiuiucore_pagerevision WHERE user_id = %s GROUP BY page_id ORDER BY max_created_at DESC LIMIT %s
    ...
    def render(self):
        return render_to_string('tuiuiuadmin/home/recent_edits.html', {
