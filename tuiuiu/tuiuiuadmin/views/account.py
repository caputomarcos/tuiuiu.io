...
from tuiuiu.tuiuiuadmin import forms
from tuiuiu.tuiuiuadmin.utils import get_available_admin_languages
from tuiuiu.tuiuiucore.models import UserPagePermissionsProxy
from tuiuiu.tuiuiuusers.forms import NotificationPreferencesForm, PreferredLanguageForm
...
def password_management_enabled():
    return getattr(settings, 'TUIUIU_PASSWORD_MANAGEMENT_ENABLED', True)


def password_reset_enabled():
    return getattr(settings, 'TUIUIU_PASSWORD_RESET_ENABLED', password_management_enabled())
...
def account(request):
    ...
    return render(request, 'tuiuiuadmin/account/account.html', {
    ...
    if can_change_password:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect('tuiuiuadmin_account')
        else:
            form = PasswordChangeForm(request.user)
    else:
        form = None

    return render(request, 'tuiuiuadmin/account/change_password.html', {
        'form': form,
...
def notification_preferences(request):
    ...
    return render(request, 'tuiuiuadmin/account/notification_preferences.html', {
        'form': form,
    })
...
def language_preferences(request):
    ...
    return render(request, 'tuiuiuadmin/account/language_preferences.html', {
        'form': form,
    })
...
@never_cache
def login(request):
    if user_is_authenticated(request.user) and request.user.has_perm('tuiuiuadmin.access_admin'):
...
def logout(request):
    response = auth_views.logout(request, next_page='tuiuiuadmin_login')
