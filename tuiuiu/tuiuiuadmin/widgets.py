...
from tuiuiu.tuiuiuadmin.datetimepicker import to_datetimepicker_format
...
class AdminDateInput(WidgetWithScript, widgets.DateInput):
    def __init__(self, attrs=None, format=None):
        fmt = format
        if fmt is None:
            fmt = getattr(settings, 'TUIUIU_DATE_FORMAT', DEFAULT_DATE_FORMAT)
...
class AdminDateTimeInput(WidgetWithScript, widgets.DateTimeInput):
    def __init__(self, attrs=None, format=None):
        fmt = format
        if fmt is None:
            fmt = getattr(settings, 'TUIUIU_DATETIME_FORMAT', DEFAULT_DATETIME_FORMAT)
        self.js_format = to_datetimepicker_format(fmt)
...
class AdminTagWidget(WidgetWithScript, TagWidget):
    def render_js_init(self, id_, name, value):
        return "initTagField({0}, {1}, {2});".format(
            json.dumps(id_),
            json.dumps(reverse('tuiuiuadmin_tag_autocomplete')),
...
class AdminPageChooser(AdminChooser):
    ...
    def render_html(self, name, value, attrs):
        ...
        return render_to_string("tuiuiuadmin/widgets/page_chooser.html", {
            'widget': self,
...
class ButtonWithDropdownFromHook(BaseDropdownMenuButton):
    template_name = 'tuiuiuadmin/pages/listing/_button_with_dropdown.html'
