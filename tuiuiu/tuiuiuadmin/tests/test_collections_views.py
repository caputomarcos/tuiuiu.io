...
class TestCollectionsIndexView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiuadmin_collections:index'), params)
...
class TestAddCollection(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiuadmin_collections:add'), params)

    def post(self, post_data={}):
        return self.client.post(reverse('tuiuiuadmin_collections:add'), post_data)
...
class TestEditCollection(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}, collection_id=None):
        return self.client.get(
            reverse('tuiuiuadmin_collections:edit', args=(collection_id or self.collection.id,)),
            params
        )
...
class TestDeleteCollection(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
        self.root_collection = Collection.get_first_root_node()
        self.collection = self.root_collection.add_child(name="Holiday snaps")
    ...
    def get(self, params={}, collection_id=None):
        return self.client.get(
            reverse('tuiuiuadmin_collections:delete', args=(collection_id or self.collection.id,)),
            params
        )
    ...
    def test_get_nonempty_collection(self):
        ...
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuadmin/collections/delete_not_empty.html')
