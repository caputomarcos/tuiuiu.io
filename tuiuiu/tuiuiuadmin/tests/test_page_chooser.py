...
from tuiuiu.tests.testapp.models import EventIndex, EventPage, SimplePage, SingleEventPage
from tuiuiu.tests.utils import TuiuiuTestUtils
from tuiuiu.tuiuiuadmin.views.chooser import can_choose_page
from tuiuiu.tuiuiucore.models import Page, UserPagePermissionsProxy


class TestChooserBrowse(TestCase, TuiuiuTestUtils):
    def setUp(self):
    ...
    def get(self, params={}):
        return self.client.get(reverse('tuiuiuadmin_choose_page'), params)
...
class TestCanChooseRootFlag(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
...
class TestChooserBrowseChild(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.root_page = Page.objects.get(id=2)
    ...
    def get(self, params={}):
        return self.client.get(reverse('tuiuiuadmin_choose_page_child',
    ...
    def test_with_blank_page_type(self):
        ...
        # (or an explicit page_type of tuiuiucore.page)
    ...
    def test_with_bad_page_type(self):
        response = self.get({'page_type': 'tuiuiucore.site'})
        self.assertEqual(response.status_code, 404)
...
class TestChooserSearch(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.root_page = Page.objects.get(id=2)
    ...
    def get(self, params=None):
        return self.client.get(reverse('tuiuiuadmin_choose_page_search'), params or {})

    def test_simple(self):
        response = self.get({'q': "foobarbaz"})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuadmin/chooser/_search_results.html')
...
class TestAutomaticRootPageDetection(TestCase, TuiuiuTestUtils):
    def setUp(self):
    ...
    def get_best_root(self, params={}):
        response = self.client.get(reverse('tuiuiuadmin_choose_page'), params)
    ...
    def test_type_page(self):
        self.assertEqual(
            self.get_best_root({'page_type': 'tuiuiucore.Page'}),
            self.tree_root)
...
class TestChooserExternalLink(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiuadmin_choose_page_external_link'), params)

    def post(self, post_data={}, url_params={}):
        url = reverse('tuiuiuadmin_choose_page_external_link')
        if url_params:
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuadmin/chooser/external_link.html')
...
class TestChooserEmailLink(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiuadmin_choose_page_email_link'), params)

    def post(self, post_data={}, url_params={}):
        url = reverse('tuiuiuadmin_choose_page_email_link')
        if url_params:
            url += '?' + urlencode(url_params)
        return self.client.post(url, post_data)
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuadmin/chooser/email_link.html')
...
class TestCanChoosePage(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
