...
from tuiuiu.tests.testapp.models import (
...
from tuiuiu.tuiuiuadmin.views.home import RecentEditsPanel
from tuiuiu.tuiuiuadmin.views.pages import PreviewOnEdit
from tuiuiu.tuiuiucore.models import GroupPagePermission, Page, PageRevision, Site
from tuiuiu.tuiuiucore.signals import page_published, page_unpublished
from tuiuiu.tuiuiusearch.index import SearchField
from tuiuiu.tuiuiuusers.models import UserProfile
...
class TestPageExplorer(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Find root page
    ...
    def test_explore(self):
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.root_page.id, )))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuadmin/pages/index.html')
    ...
    def test_explore_root(self):
        response = self.client.get(reverse('tuiuiuadmin_explore_root'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuadmin/pages/index.html')
    ...
    def test_pagination(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.root_page.id, )), {'p': 2})
    ...
    def test_pagination_invalid(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.root_page.id, )), {'p': 'Hello World!'})
    ...
    def test_pagination_out_of_range(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.root_page.id, )), {'p': 99999})
    ...
    def test_listing_uses_admin_display_title(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.new_event.id, )))
        self.assertContains(response, 'New event (single event)')

    def test_parent_page_is_specific(self):
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.child_page.id, )))
        self.assertEqual(response.status_code, 200)
    ...
    def test_explorer_no_perms(self):
        ...
        admin = reverse('tuiuiuadmin_home')
        self.assertRedirects(
            self.client.get(reverse('tuiuiuadmin_explore', args=(self.root_page.id, ))),
            admin)
        self.assertRedirects(
            self.client.get(reverse('tuiuiuadmin_explore_root')), admin)


class TestPageExplorerSignposting(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
    ...
    def test_admin_at_root(self):
        ...
        self.assertContains(
            response,
            (
                "The root level is where you can add new sites to your Tuiuiu installation. "
    ...
    def test_admin_at_non_site_page(self):
        self.assertTrue(self.client.login(username='superuser', password='password'))
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.no_site_page.id, )))
        self.assertEqual(response.status_code, 200)
    ...
    def test_admin_at_site_page(self):
        self.assertTrue(self.client.login(username='superuser', password='password'))
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.site_page.id, )))
        self.assertEqual(response.status_code, 200)
...
class TestExplorablePageVisibility(TestCase, TuiuiuTestUtils):
    """
    ...
    User "mary" is is no Groups, but she has the "access tuiuiu admin" permission.
    ...
    def test_admin_can_explore_every_page(self):
        ...
        for page in Page.objects.all():
            response = self.client.get(reverse('tuiuiuadmin_explore', args=[page.pk]))
            self.assertEqual(response.status_code, 200)
    ...
    def test_admin_sees_breadcrumbs_up_to_root_page(self):
        self.assertTrue(self.client.login(username='superman', password='password'))
        response = self.client.get(reverse('tuiuiuadmin_explore', args=[6]))
        self.assertEqual(response.status_code, 200)
...
class TestPageCreation(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def test_add_subpage(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:add_subpage', args=(self.root_page.id, )))
        self.assertEqual(response.status_code, 200)

        self.assertContains(response, "Simple page")
        target_url = reverse('tuiuiuadmin_pages:add', args=('tests', 'simplepage', self.root_page.id))
    ...
    def test_add_subpage_with_subpage_types(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_pages:add_subpage', args=(business_index.id, )))
        self.assertEqual(response.status_code, 200)
    ...
    def test_add_subpage_with_one_valid_subpage_type(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_pages:add_subpage', args=(business_subindex.id, )))
        # Should be redirected to the 'add' page for BusinessChild, the only valid subpage type
        self.assertRedirects(
            response,
            reverse('tuiuiuadmin_pages:add', args=('tests', 'businesschild', business_subindex.id))
        )
    ...
    def test_add_subpage_nonexistantparent(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:add_subpage', args=(100000, )))
        self.assertEqual(response.status_code, 404)

    def test_add_subpage_with_next_param(self):
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add_subpage', args=(self.root_page.id, )),
            {'next': '/admin/users/'}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_simplepage(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:add', args=('tests', 'simplepage', self.root_page.id)))
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_multipart(self):
        ...
        self.assertTemplateUsed(response, 'tuiuiuadmin/pages/create.html')

        # check for FilePage which has file field
        response = self.client.get(reverse('tuiuiuadmin_pages:add', args=('tests', 'filepage', self.root_page.id)))
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_page_without_promote_tab(self):
        ...
        """
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'standardindex', self.root_page.id))
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, '<a href="#tab-content" class="active">Content</a>')
    ...
    def test_create_page_with_custom_tabs(self):
        ...
        """
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'standardchild', self.root_page.id))
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, '<a href="#tab-content" class="active">Content</a>')
        self.assertContains(response, '<a href="#tab-promote" class="">Promote</a>')
    ...
    def test_cannot_create_page_with_is_creatable_false(self):
        ...
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'mtibasepage', self.root_page.id))
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_cannot_create_page_when_can_create_at_returns_false(self):
        ...
        add_url = reverse('tuiuiuadmin_pages:add', args=[
    ...
    def test_cannot_create_page_with_wrong_parent_page_types(self):
        ...
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'businesschild', self.root_page.id))
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_cannot_create_page_with_wrong_subpage_types(self):
        ...
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'simplepage', business_index.id))
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_create_simplepage_post(self):
        ...
        response = self.client.post(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'simplepage', self.root_page.id)),
            post_data
        )
        ...
        self.assertRedirects(response, reverse('tuiuiuadmin_pages:edit', args=(page.id, )))
    ...
    def test_create_simplepage_scheduled(self):
        ...
        response = self.client.post(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'simplepage', self.root_page.id)), post_data
        )
    ...
    def test_create_simplepage_post_publish(self):
        ...
        self.assertRedirects(response, reverse('tuiuiuadmin_explore', args=(self.root_page.id, )))
    ...
    def test_create_nonexistantparent(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:add', args=('tests', 'simplepage', 100000)))
        self.assertEqual(response.status_code, 404)

    def test_create_nonpagetype(self):
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add', args=('tuiuiuimages', 'image', self.root_page.id))
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_preview_on_create(self):
        ...
        preview_url = reverse('tuiuiuadmin_pages:preview_on_add',
    ...
    def test_whitespace_titles_with_tab(self):
        ...
        response = self.client.post(reverse('tuiuiuadmin_pages:add', args=('tests', 'simplepage', self.root_page.id)), post_data)
...
class TestPageEdit(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def test_page_edit(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_pages:edit', args=(self.event_page.id, )))
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_multipart(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertNotContains(response, 'enctype="multipart/form-data"')
        self.assertTemplateUsed(response, 'tuiuiuadmin/pages/edit.html')

        # check for FilePage which has file field
        response = self.client.get(reverse('tuiuiuadmin_pages:edit', args=(self.file_page.id, )))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'enctype="multipart/form-data"')
    ...
    def test_upload_file_publish(self):
        ...
        response = self.client.post(reverse('tuiuiuadmin_pages:edit', args=[self.file_page.id]), post_data)
    ...
    def test_upload_file_draft(self):
        ...
        # Should be redirected to edit page
        self.assertRedirects(response, reverse('tuiuiuadmin_pages:edit', args=[self.file_page.id]))
    ...
    def test_page_edit_bad_permissions(self):
        ...
        # Get edit page
        response = self.client.get(reverse('tuiuiuadmin_pages:edit', args=(self.child_page.id, )))
    ...
    def test_page_edit_post(self):
        ...
        response = self.client.post(reverse('tuiuiuadmin_pages:edit', args=(self.child_page.id, )), post_data)

        # Should be redirected to edit page
        self.assertRedirects(response, reverse('tuiuiuadmin_pages:edit', args=(self.child_page.id, )))
    ...
    def test_page_edit_post_publish(self):
        ...
        response = self.client.post(
            reverse('tuiuiuadmin_pages:edit', args=(self.child_page.id, )), post_data, follow=True
        )
    ...
    def test_first_published_at_editable(self):
        ...
        self.client.post(reverse('tuiuiuadmin_pages:edit', args=(self.child_page.id, )), post_data)
    ...
    def test_preview_on_edit(self):
        ...
        preview_url = reverse('tuiuiuadmin_pages:preview_on_edit',
    ...
    def test_preview_on_edit_no_session_key(self):
        ...
        self.assertContains(
            response,
            "<title>Tuiuiu - Preview error</title>",
            html=True
        )
        self.assertContains(
            response,
    ...
    def test_before_edit_page_hook_post(self):
        ...
        with self.register_hook('before_edit_page', hook_func):
            ...
            response = self.client.post(
                reverse('tuiuiuadmin_pages:edit', args=(self.child_page.id, )), post_data
            )
...
class TestPageEditReordering(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def test_reorder(self):
        ...
        response = self.client.post(reverse('tuiuiuadmin_pages:edit', args=(self.event_page.id, )), post_data)

        # Should be redirected back to same page
        self.assertRedirects(response, reverse('tuiuiuadmin_pages:edit', args=(self.event_page.id, )))
...
class TestPageDelete(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def test_page_delete(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:delete', args=(self.child_page.id, )))
        self.assertEqual(response.status_code, 200)
    ...
    def test_page_delete_post(self):
        ...
        # Post
        response = self.client.post(reverse('tuiuiuadmin_pages:delete', args=(self.child_page.id, )))
    ...
    def test_subpage_deletion(self):
        ...
        # Post
        response = self.client.post(reverse('tuiuiuadmin_pages:delete', args=(self.child_index.id, )))
...
class TestPageSearch(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.user = self.login()

    def get(self, params=None, **extra):
        return self.client.get(reverse('tuiuiuadmin_pages:search'), params or {}, **extra)

    def test_view(self):
        response = self.get()
        self.assertTemplateUsed(response, 'tuiuiuadmin/pages/search.html')
        self.assertEqual(response.status_code, 200)
    ...
    def test_ajax(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateNotUsed(response, 'tuiuiuadmin/pages/search.html')
        self.assertTemplateUsed(response, 'tuiuiuadmin/pages/search_results.html')
        self.assertEqual(response.context['query_string'], "Hello")
...
class TestPageMove(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def test_page_move(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:move', args=(self.test_page.id, )))
        self.assertEqual(response.status_code, 200)
    ...
    def test_page_move_confirm(self):
        response = self.client.get(
            reverse('tuiuiuadmin_pages:move_confirm', args=(self.test_page.id, self.section_b.id))
        )
        self.assertEqual(response.status_code, 200)

    def test_page_set_page_position(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:set_page_position', args=(self.test_page.id, )))
        self.assertEqual(response.status_code, 200)


class TestPageCopy(TestCase, TuiuiuTestUtils):
    ...
    def test_page_copy(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:copy', args=(self.test_page.id, )))

        # Check response
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuadmin/pages/copy.html')
    ...
    def test_page_copy_bad_permissions(self):
        ...
        response = self.client.post(reverse('tuiuiuadmin_pages:copy', args=(self.test_page.id, )), post_data)
    ...
    def test_page_copy_post_new_parent(self):
        ...
        self.assertRedirects(response, reverse('tuiuiuadmin_explore', args=(self.test_child_page.id, )))
    ...
    def test_page_copy_post_existing_slug_within_same_parent_page(self):
        ...
        self.assertFormError(
            ...
            'form',
            'new_slug',
            "This slug is already in use within the context of its parent page \"Welcome to your new Tuiuiu site!\""
        )
...
class TestPageUnpublish(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.user = self.login()
    ...
    def test_unpublish_view(self):
        ...
        """
        # Get unpublish page
        response = self.client.get(reverse('tuiuiuadmin_pages:unpublish', args=(self.page.id, )))

        # Check that the user received an unpublish confirm page
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuadmin/pages/confirm_unpublish.html')
    ...
    def test_unpublish_view_invalid_page_id(self):
        ...
        """
        # Get unpublish page
        response = self.client.get(reverse('tuiuiuadmin_pages:unpublish', args=(12345, )))
    ...
    def test_unpublish_view_post(self):
        ...
        response = self.client.post(reverse('tuiuiuadmin_pages:unpublish', args=(self.page.id, )))
...
class TestPageUnpublishIncludingDescendants(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.user = self.login()
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def test_unpublish_descendants_view(self):
        ...
        """
        # Get unpublish page
        response = self.client.get(reverse('tuiuiuadmin_pages:unpublish', args=(self.test_page.id, )))
    ...
    def test_unpublish_include_children_view_post(self):
        ...
        """
        # Post to the unpublish page
        response = self.client.post(reverse('tuiuiuadmin_pages:unpublish', args=(self.test_page.id, )), {'include_descendants': 'on'})
    ...
    def test_unpublish_not_include_children_view_post(self):
        ...
        """
        # Post to the unpublish page
        response = self.client.post(reverse('tuiuiuadmin_pages:unpublish', args=(self.test_page.id, )), {})
...
class TestApproveRejectModeration(TestCase, TuiuiuTestUtils):
    def setUp(self):
    ...
    def test_approve_moderation_view(self):
        ...
        # Post
        response = self.client.post(reverse('tuiuiuadmin_pages:approve_moderation', args=(self.revision.id, )))
    ...
    def test_approve_moderation_view_bad_revision_id(self):
        ...
        """
        # Post
        response = self.client.post(reverse('tuiuiuadmin_pages:approve_moderation', args=(12345, )))
    ...
    def test_reject_moderation_view(self):
        ...
        """
        # Post
        response = self.client.post(reverse('tuiuiuadmin_pages:reject_moderation', args=(self.revision.id, )))
    ...
    def test_reject_moderation_view_bad_revision_id(self):
        ...
        """
        # Post
        response = self.client.post(reverse('tuiuiuadmin_pages:reject_moderation', args=(12345, )))
    ...
    def test_preview_for_moderation(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:preview_for_moderation', args=(self.revision.id, )))
...
class TestContentTypeUse(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
    ...
    def test_content_type_use(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_pages:type_use', args=('tests', 'eventpage')))

        # Check response
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuadmin/pages/content_type_use.html')
        self.assertContains(response, "Christmas")


class TestSubpageBusinessRules(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def test_standard_subpage(self):
        add_subpage_url = reverse('tuiuiuadmin_pages:add_subpage', args=(self.standard_index.id, ))

        # explorer should contain a link to 'add child page'
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.standard_index.id, )))
        self.assertEqual(response.status_code, 200)
    ...
    def test_business_subpage(self):
        add_subpage_url = reverse('tuiuiuadmin_pages:add_subpage', args=(self.business_index.id, ))

        # explorer should contain a link to 'add child page'
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.business_index.id, )))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, add_subpage_url)
    ...
    def test_business_child_subpage(self):
        add_subpage_url = reverse('tuiuiuadmin_pages:add_subpage', args=(self.business_child.id, ))

        # explorer should not contain a link to 'add child page', as this page doesn't accept subpages
        response = self.client.get(reverse('tuiuiuadmin_explore', args=(self.business_child.id, )))
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(reverse('tuiuiuadmin_pages:add_subpage', args=(self.business_child.id, )))
        self.assertEqual(response.status_code, 403)
    ...
    def test_cannot_add_invalid_subpage_type(self):
        ...
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'standardchild', self.business_index.id))
        )
        self.assertEqual(response.status_code, 403)

        # likewise for BusinessChild which has an empty subpage_types list
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'standardchild', self.business_child.id))
        )
        self.assertEqual(response.status_code, 403)

        # cannot add BusinessChild to StandardIndex, as BusinessChild restricts is parent page types
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'businesschild', self.standard_index.id))
        )
        self.assertEqual(response.status_code, 403)

        # but we can add a BusinessChild to BusinessIndex
        response = self.client.get(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'businesschild', self.business_index.id))
        )
        self.assertEqual(response.status_code, 200)

    def test_not_prompted_for_page_type_when_only_one_choice(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:add_subpage', args=(self.business_subindex.id, )))
        # BusinessChild is the only valid subpage type of BusinessSubIndex, so redirect straight there
        self.assertRedirects(
            response, reverse('tuiuiuadmin_pages:add', args=('tests', 'businesschild', self.business_subindex.id))
        )


class TestNotificationPreferences(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def submit(self):
        return self.client.post(reverse('tuiuiuadmin_pages:edit', args=(self.child_page.id, )), self.post_data)
    ...
    def approve(self):
        return self.client.post(reverse('tuiuiuadmin_pages:approve_moderation', args=(self.revision.id, )))

    def reject(self):
        return self.client.post(reverse('tuiuiuadmin_pages:reject_moderation', args=(self.revision.id, )))
    ...
    def test_moderator_group_notifications(self):
        ...
        # Submit
        # This used to break in Tuiuiu 1.3 (Postgres exception, SQLite 3/4 notifications)
    ...
    @mock.patch('tuiuiu.tuiuiuadmin.utils.django_send_mail', side_effect=IOError('Server down'))
...
class TestLocking(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def test_lock_post(self):
        response = self.client.post(reverse('tuiuiuadmin_pages:lock', args=(self.child_page.id, )))
    ...
    def test_lock_get(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:lock', args=(self.child_page.id, )))
    ...
    def test_lock_post_with_good_redirect(self):
        response = self.client.post(reverse('tuiuiuadmin_pages:lock', args=(self.child_page.id, )), {
            'next': reverse('tuiuiuadmin_pages:edit', args=(self.child_page.id, ))
        })
    ...
    def test_lock_post_bad_page(self):
        response = self.client.post(reverse('tuiuiuadmin_pages:lock', args=(9999, )))
    ...
    def test_unlock_post(self):
        ...
        response = self.client.post(reverse('tuiuiuadmin_pages:unlock', args=(self.child_page.id, )))
    ...
    def test_unlock_get(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_pages:unlock', args=(self.child_page.id, )))
    ...
    def test_unlock_post_bad_page(self):
        ...
        response = self.client.post(reverse('tuiuiuadmin_pages:unlock', args=(9999, )))
...
class TestIssue197(TestCase, TuiuiuTestUtils):
    def test_issue_197(self):
        ...
        response = self.client.post(reverse('tuiuiuadmin_pages:edit', args=(self.tagged_page.id, )), post_data)
...
class TestChildRelationsOnSuperclass(TestCase, TuiuiuTestUtils):
    ...
    def test_post_create_form(self):
        ...
        response = self.client.post(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'standardindex', self.root_page.id)), post_data
        )
    ...
    def test_get_edit_form(self):
        response = self.client.get(reverse('tuiuiuadmin_pages:edit', args=(self.index_page.id, )))
        self.assertEqual(response.status_code, 200)
    ...
    def test_post_edit_form(self):
        ...
        response = self.client.post(reverse('tuiuiuadmin_pages:edit', args=(self.index_page.id, )), post_data)
...
class TestRevisions(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
...
class TestCompareRevisions(TestCase, TuiuiuTestUtils):
    ...
    def test_compare_revisions(self):
        compare_url = reverse(
            'tuiuiuadmin_pages:revisions_compare',
...
class TestIssue2599(TestCase, TuiuiuTestUtils):
    """
...
class TestIssue2492(TestCase, TuiuiuTestUtils):
    """
    ...
    def test_page_edit_post_publish_url(self):
        ...
        response = self.client.post(
            reverse('tuiuiuadmin_pages:edit', args=(self.child_page.id, )),
...
class TestInlinePanelMedia(TestCase, TuiuiuTestUtils):
    """
    ...
    def test_inline_panel_media(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_pages:add', args=('tests', 'simplepage', homepage.id)))
        self.assertEqual(response.status_code, 200)
        self.assertNotContains(response, 'tuiuiuadmin/js/hallo-bootstrap.js')

        # but sectionedrichtextpage does
        response = self.client.get(reverse('tuiuiuadmin_pages:add', args=('tests', 'sectionedrichtextpage', homepage.id)))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'tuiuiuadmin/js/hallo-bootstrap.js')


class TestInlineStreamField(TestCase, TuiuiuTestUtils):
    """
    ...
    def test_inline_streamfield(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_pages:add', args=('tests', 'inlinestreampage', homepage.id)))
        self.assertEqual(response.status_code, 200)
...
class TestRecentEditsPanel(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Find root page
        self.root_page = Page.objects.get(id=2)
...
class TestIssue2994(TestCase, TuiuiuTestUtils):
    """
    ...
    def test_page_edit_post_publish_url(self):
        ...
        self.client.post(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'defaultstreampage', self.root_page.id)), post_data
        )
...
class TestParentalM2M(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
    ...
    def test_create_and_save(self):
        ...
        response = self.client.post(
            reverse('tuiuiuadmin_pages:add', args=('tests', 'eventpage', self.events_index.id)),
            post_data
        )
        created_page = EventPage.objects.get(url_path='/home/events/presidents-day/')
        self.assertRedirects(response, reverse('tuiuiuadmin_pages:edit', args=(created_page.id, )))
    ...
    def test_create_and_publish(self):
        ...
        self.assertRedirects(response, reverse('tuiuiuadmin_explore', args=(self.events_index.id, )))
    ...
    def test_edit_and_save(self):
        ...
        response = self.client.post(
            reverse('tuiuiuadmin_pages:edit', args=(self.christmas_page.id, )),
            post_data
        )
        self.assertRedirects(response, reverse('tuiuiuadmin_pages:edit', args=(self.christmas_page.id, )))
...
class TestValidationErrorMessages(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
...
class TestDraftAccess(TestCase, TuiuiuTestUtils):
    ...
    def test_draft_access_admin(self):
        ...
        response = self.client.get(reverse('tuiuiuadmin_pages:view_draft', args=(self.child_page.id, )))
...
class TestPreview(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
