...
from tuiuiu.tests.testapp.forms import ValidatedPageForm
...
from tuiuiu.tuiuiuadmin.forms import TuiuiuAdminModelForm, TuiuiuAdminPageForm
from tuiuiu.tuiuiuadmin.rich_text import HalloRichTextArea
from tuiuiu.tuiuiuadmin.widgets import AdminAutoHeightTextInput, AdminDateInput, AdminPageChooser
...
class TestGetFormForModel(TestCase):
    def test_get_form_for_model(self):
        EventPageForm = get_form_for_model(EventPage, form_class=TuiuiuAdminPageForm)
        form = EventPageForm()
    ...
    def test_direct_form_field_overrides(self):
        ...
        SimplePageForm = get_form_for_model(SimplePage, form_class=TuiuiuAdminPageForm)
    ...
    def test_get_form_for_model_with_specific_fields(self):
        EventPageForm = get_form_for_model(
            EventPage, form_class=TuiuiuAdminPageForm, fields=['date_from'],
            formsets=['speakers'])
        form = EventPageForm()
    ...
    def test_get_form_for_model_with_excluded_fields(self):
        EventPageForm = get_form_for_model(
            EventPage, form_class=TuiuiuAdminPageForm,
...
class TestPageEditHandlers(TestCase):
    ...
    def test_check_invalid_base_form_class(self):
        ...
        invalid_base_form = checks.Error(
            "ValidatedPage.base_form_class does not extend TuiuiuAdminPageForm",
            hint="Ensure that tuiuiu.tuiuiuadmin.tests.test_edit_handlers.BadFormClass extends TuiuiuAdminPageForm",
            obj=ValidatedPage,
            id='tuiuiuadmin.E001')

        invalid_edit_handler = checks.Error(
            "ValidatedPage.get_edit_handler().get_form_class(ValidatedPage) does not extend TuiuiuAdminPageForm",
            hint="Ensure that the EditHandler for ValidatedPage creates a subclass of TuiuiuAdminPageForm",
            obj=ValidatedPage,
            id='tuiuiuadmin.E002')
        ...
        with mock.patch.object(ValidatedPage, 'base_form_class', new=BadFormClass):
            ...
            # don't build the CSS)
            errors = [e for e in errors if e.id != 'tuiuiuadmin.W001']
    ...
    def test_custom_edit_handler_form_class(self):
        ...
        with mock.patch.object(ValidatedPage, 'edit_handler', new=TabbedInterface([]), create=True):
            ...
            self.assertTrue(issubclass(form_class, TuiuiuAdminPageForm))
...
class TestFieldPanel(TestCase):
    def setUp(self):
        self.EventPageForm = get_form_for_model(
            EventPage, form_class=TuiuiuAdminPageForm, formsets=[])
...
class TestPageChooserPanel(TestCase):
    ...
    def test_render_js_init(self):
        ...
        expected_js = 'createPageChooser("{id}", ["{model}"], {parent}, false, null);'.format(
            id="id_page", model="tuiuiucore.page", parent=self.events_index_page.id)
...
class TestInlinePanel(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
