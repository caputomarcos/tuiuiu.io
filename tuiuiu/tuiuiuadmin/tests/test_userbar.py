...
from tuiuiu.tests.testapp.models import BusinessChild, BusinessIndex
from tuiuiu.tests.utils import TuiuiuTestUtils
from tuiuiu.tuiuiucore.models import PAGE_TEMPLATE_VAR, Page, Site
...
class TestUserbarTag(TestCase):
    ...
    def test_userbar_tag(self):
        template = Template("{% load tuiuiuuserbar %}{% tuiuiuuserbar %}")
        content = template.render(Context({
            PAGE_TEMPLATE_VAR: self.homepage,
            'request': self.dummy_request(self.user),
        }))

        self.assertIn("<!-- Tuiuiu user bar embed code -->", content)

    def test_userbar_does_not_break_without_request(self):
        template = Template("{% load tuiuiuuserbar %}{% tuiuiuuserbar %}boom")
        content = template.render(Context({
        }))
...
class TestUserbarFrontend(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
        self.homepage = Page.objects.get(id=2)

    def test_userbar_frontend(self):
        response = self.client.get(reverse('tuiuiuadmin_userbar_frontend', args=(self.homepage.id, )))

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuadmin/userbar/base.html')
...
class TestUserbarAddLink(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
    ...
    def test_page_allowing_subpages(self):
        response = self.client.get(reverse('tuiuiuadmin_userbar_frontend', args=(self.event_index.id, )))

        # page allows subpages, so the 'add page' button should show
        expected_url = reverse('tuiuiuadmin_pages:add_subpage', args=(self.event_index.id, ))
        expected_link = '<a href="%s" target="_parent">Add a child page</a>' \
            % expected_url
        self.assertContains(response, expected_link)

    def test_page_disallowing_subpages(self):
        response = self.client.get(reverse('tuiuiuadmin_userbar_frontend', args=(self.business_child.id, )))

        # page disallows subpages, so the 'add page' button shouldn't show
        expected_url = reverse('tuiuiuadmin_pages:add_subpage', args=(self.business_index.id, ))
        expected_link = '<a href="%s" target="_parent">Add a child page</a>' \
            % expected_url
        self.assertNotContains(response, expected_link)


class TestUserbarModeration(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
        self.homepage = Page.objects.get(id=2)
    ...
    def test_userbar_moderation(self):
        response = self.client.get(reverse('tuiuiuadmin_userbar_moderation', args=(self.revision.id, )))
