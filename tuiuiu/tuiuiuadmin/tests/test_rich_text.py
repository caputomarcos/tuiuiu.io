...
from tuiuiu.tests.testapp.models import SingleEventPage
from tuiuiu.tests.testapp.rich_text import CustomRichTextArea
from tuiuiu.tests.utils import TuiuiuTestUtils
from tuiuiu.tuiuiuadmin.rich_text import HalloRichTextArea, get_rich_text_editor_widget
from tuiuiu.tuiuiucore.models import Page, get_page_models
...
class BaseRichTextEditHandlerTestCase(TestCase):
    def _clear_edit_handler_cache(self):
        ...
        """
        from tuiuiu.tests.testapp.models import DefaultRichBlockFieldPage
...
class TestGetRichTextEditorWidget(TestCase):
    ...
    def test_default(self):
        ...
        if hasattr(settings, 'TUIUIUADMIN_RICH_TEXT_EDITORS'):
            del settings.TUIUIUADMIN_RICH_TEXT_EDITORS

        self.assertIsInstance(get_rich_text_editor_widget(), HalloRichTextArea)

    @override_settings(TUIUIUADMIN_RICH_TEXT_EDITORS={
        'default': {
...
@override_settings()
class TestDefaultRichText(BaseRichTextEditHandlerTestCase, TuiuiuTestUtils):
    ...
    def test_default_editor_in_rich_text_field(self):
        response = self.client.get(reverse(
            'tuiuiuadmin_pages:add', args=('tests', 'defaultrichtextfieldpage', self.root_page.id)
        ))
    ...
    def test_default_editor_in_rich_text_block(self):
        response = self.client.get(reverse(
            'tuiuiuadmin_pages:add', args=('tests', 'defaultrichblockfieldpage', self.root_page.id)
        ))
...
class TestOverriddenDefaultRichText(BaseRichTextEditHandlerTestCase, TuiuiuTestUtils):
...
class TestCustomDefaultRichText(BaseRichTextEditHandlerTestCase, TuiuiuTestUtils):
    ...
    def test_custom_editor_in_rich_text_field(self):
        response = self.client.get(reverse(
            'tuiuiuadmin_pages:add', args=('tests', 'customrichtextfieldpage', self.root_page.id)
        ))
    ...
    def test_custom_editor_in_rich_text_block(self):
        response = self.client.get(reverse(
            'tuiuiuadmin_pages:add', args=('tests', 'customrichblockfieldpage', self.root_page.id)
        ))
