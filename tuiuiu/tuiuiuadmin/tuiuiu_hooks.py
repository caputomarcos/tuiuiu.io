...
from tuiuiu.tuiuiuadmin.menu import MenuItem, SubmenuMenuItem, settings_menu
from tuiuiu.tuiuiuadmin.navigation import get_explorable_root_page
from tuiuiu.tuiuiuadmin.search import SearchArea
from tuiuiu.tuiuiuadmin.utils import user_has_any_page_permission
from tuiuiu.tuiuiuadmin.viewsets import viewsets
from tuiuiu.tuiuiuadmin.widgets import Button, ButtonWithDropdownFromHook, PageListingButton
from tuiuiu.tuiuiucore import hooks
from tuiuiu.tuiuiucore.permissions import collection_permission_policy


class ExplorerMenuItem(MenuItem):
    template = 'tuiuiuadmin/shared/explorer_menu_item.html'
...
def register_explorer_menu_item():
    return ExplorerMenuItem(
        _('Pages'), reverse('tuiuiuadmin_explore_root'),
        name='explorer',
...
class SettingsMenuItem(SubmenuMenuItem):
    template = 'tuiuiuadmin/shared/menu_settings_menu_item.html'
...
def register_permissions():
    return Permission.objects.filter(content_type__app_label='tuiuiuadmin', codename='access_admin')


class PageSearchArea(SearchArea):
    def __init__(self):
        super(PageSearchArea, self).__init__(
            _('Pages'), reverse('tuiuiuadmin_pages:search'),
            name='pages',
            classnames='icon icon-folder-open-inverse',
            order=100)
...
def register_collections_menu_item():
    return CollectionsMenuItem(_('Collections'), reverse('tuiuiuadmin_collections:index'), classnames='icon icon-folder-open-1', order=700)
...
def page_listing_buttons(page, page_perms, is_parent=False):
    ...
    if page_perms.can_add_subpage():
        if is_parent:
            yield Button(
                _('Add child page'),
                reverse('tuiuiuadmin_pages:add_subpage', args=[page.id]),
...
def page_listing_more_buttons(page, page_perms, is_parent=False):
    if page_perms.can_move():
        yield Button(
            _('Move'),
            reverse('tuiuiuadmin_pages:move', args=[page.id]),
    ...
    if not page.is_root():
        yield Button(
            _('Copy'),
            reverse('tuiuiuadmin_pages:copy', args=[page.id]),
    ...
    if page_perms.can_delete():
        yield Button(
            _('Delete'),
            reverse('tuiuiuadmin_pages:delete', args=[page.id]),
    ...
    if page_perms.can_unpublish():
        yield Button(
            _('Unpublish'),
            reverse('tuiuiuadmin_pages:unpublish', args=[page.id]),
    ...
    if not page.is_root():
        yield Button(
            _('Revisions'),
            reverse('tuiuiuadmin_pages:revisions_index', args=[page.id]),
