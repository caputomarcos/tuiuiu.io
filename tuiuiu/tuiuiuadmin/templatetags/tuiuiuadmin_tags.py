...
from tuiuiu.utils.pagination import DEFAULT_PAGE_KEY, replace_page_in_query
from tuiuiu.tuiuiuadmin.menu import admin_menu
from tuiuiu.tuiuiuadmin.navigation import get_explorable_root_page
from tuiuiu.tuiuiuadmin.search import admin_search_areas
...
from tuiuiu.tuiuiucore.utils import cautious_slugify as _cautious_slugify
from tuiuiu.tuiuiucore.utils import camelcase_to_underscore, escape_script
...
def menu_search(context):
    ...
    return render_to_string('tuiuiuadmin/shared/menu_search.html', {
        'search_url': search_area.url,
    })


@register.inclusion_tag('tuiuiuadmin/shared/main_nav.html', takes_context=True)
...
@register.inclusion_tag('tuiuiuadmin/shared/breadcrumb.html', takes_context=True)
...
@register.inclusion_tag('tuiuiuadmin/shared/search_other.html', takes_context=True)
...
def usage_count_enabled():
    return getattr(settings, 'TUIUIU_USAGE_COUNT_ENABLED', False)
...
def allow_unicode_slugs():
    if django.VERSION < (1, 9):
        ...
        return getattr(settings, 'TUIUIU_ALLOW_UNICODE_SLUGS', True)


@assignment_tag
def auto_update_preview():
    return getattr(settings, 'TUIUIU_AUTO_UPDATE_PREVIEW', False)
...
@register.inclusion_tag("tuiuiuadmin/pages/listing/_pagination.html",
                        takes_context=True)
def paginate(context, page, base_url='', page_key=DEFAULT_PAGE_KEY,
    ...
    page_key
        ...
        name as used in the :func:`~tuiuiu.utils.pagination.paginate`
        function.
...
@register.inclusion_tag("tuiuiuadmin/pages/listing/_buttons.html",
                        takes_context=True)
