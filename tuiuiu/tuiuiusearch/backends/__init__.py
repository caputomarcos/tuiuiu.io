...
def get_search_backend_config():
    search_backends = getattr(settings, 'TUIUIUSEARCH_BACKENDS', {})
...
def import_backend(dotted_path):
    ...
    eg:
      old: tuiuiu.tuiuiusearch.backends.elasticsearch.ElasticsearchSearchBackend
      new: tuiuiu.tuiuiusearch.backends.elasticsearch
...
def get_search_backend(backend='default', **kwargs):
    ...
    try:
        # Try to get the TUIUIUSEARCH_BACKENDS entry for the given backend name first
...
def _backend_requires_auto_update(backend_name, params):
    ...
    # _TUIUIUSEARCH_FORCE_AUTO_UPDATE is only used by Tuiuiu tests. It allows
    # us to test AUTO_UPDATE behaviour against Elasticsearch without having to
    # have AUTO_UPDATE enabed for every test.
    force_auto_update = getattr(settings, '_TUIUIUSEARCH_FORCE_AUTO_UPDATE', [])
