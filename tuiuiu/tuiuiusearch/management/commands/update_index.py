...
from tuiuiu.tuiuiusearch.index import get_indexed_models
...
def group_models_by_index(backend, models):
    ...
    ... ])
    {
        <Index tuiuiucore_page>: [tuiuiucore.Page, myapp.HomePage, myapp.StandardPage],
        <Index tuiuiuimages_image>: [tuiuiuimages.Image],
    }
    """
    indices = {}
...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        elif hasattr(settings, 'TUIUIUSEARCH_BACKENDS'):
            # index all backends listed in settings
            backend_names = settings.TUIUIUSEARCH_BACKENDS.keys()
        else:
