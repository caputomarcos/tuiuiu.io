...
@mock.patch('tuiuiu.tuiuiusearch.tests.DummySearchBackend', create=True)
@override_settings(TUIUIUSEARCH_BACKENDS={
    'default': {
        'BACKEND': 'tuiuiu.tuiuiusearch.tests.DummySearchBackend'
    }
})
class TestInsertOrUpdateObject(TestCase, TuiuiuTestUtils):
    ...
    def test_catches_index_error(self, backend):
        ...
        with self.assertLogs('tuiuiu.search.index', level='ERROR') as cm:
            index.insert_or_update_object(obj)
...
class TestRemoveObject(TestCase, TuiuiuTestUtils):
...
class TestSignalHandlers(TestCase, TuiuiuTestUtils):
