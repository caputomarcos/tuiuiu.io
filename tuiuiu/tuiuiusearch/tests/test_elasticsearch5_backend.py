...
from tuiuiu.tuiuiusearch.backends.elasticsearch5 import Elasticsearch5SearchBackend

from .test_backends import BackendTests


class TestElasticsearch5SearchBackend(BackendTests, TestCase):
    backend_path = 'tuiuiu.tuiuiusearch.backends.elasticsearch5'
    ...
    def test_filter_with_unsupported_lookup_type(self):
        from tuiuiu.tuiuiusearch.backends.base import FilterError
    ...
    def test_search_with_hyphen(self):
        ...
        See: https://github.com/tuiuiu/tuiuiu/issues/937
        """
        # Reset the index
        self.reset_index()
        self.backend.add_type(models.SearchTest)
...
class TestElasticsearch5SearchQuery(TestCase):
    ...
    def test_and_filter(self):
        ...
        expected_result = {'bool': {'filter': [
            {'match': {'content_type': 'searchtests.SearchTest'}},
            {'bool': {'must': [{'term': {'live_filter': True}}, {'term': {'title_filter': 'Test'}}]}}
        ], 'must': {'multi_match': {'query': 'Hello', 'fields': ['_all', '_partials']}}}}
    ...
    def test_or_filter(self):
        ...
        expected_result = {'bool': {'filter': [
            {'match': {'content_type': 'searchtests.SearchTest'}},
            {'bool': {'should': [{'term': {'live_filter': True}}, {'term': {'title_filter': 'Test'}}]}}
        ], 'must': {'multi_match': {'query': 'Hello', 'fields': ['_all', '_partials']}}}}
        self.assertDictEqual(query, expected_result)
    ...
    def test_negated_filter(self):
        ...
        expected_result = {'bool': {'filter': [
            {'match': {'content_type': 'searchtests.SearchTest'}},
            {'bool': {'mustNot': {'term': {'live_filter': True}}}}
        ], 'must': {'multi_match': {'query': 'Hello', 'fields': ['_all', '_partials']}}}}
        self.assertDictEqual(query.get_query(), expected_result)
...
class TestElasticsearch5SearchResults(TestCase):
    ...
    def test_basic_search(self, search):
        ...
        search.assert_any_call(
            ...
            _source=False,
            stored_fields='pk',
            index='tuiuiu__searchtests_searchtest'
        )
...
class TestElasticsearch5Mapping(TestCase):
    ...
    def setUp(self):
        # Create ES mapping
        self.es_mapping = Elasticsearch5SearchBackend.mapping_class(models.SearchTest)
...
class TestElasticsearch5MappingInheritance(TestCase):
    ...
    def setUp(self):
        # Create ES mapping
        self.es_mapping = Elasticsearch5SearchBackend.mapping_class(models.SearchTestChild)
