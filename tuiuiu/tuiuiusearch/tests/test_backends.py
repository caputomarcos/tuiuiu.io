...
from tuiuiu.tests.search import models
from tuiuiu.tests.utils import TuiuiuTestUtils
from tuiuiu.tuiuiusearch.backends import (
    InvalidSearchBackendError, get_search_backend, get_search_backends)
from tuiuiu.tuiuiusearch.backends.base import FieldError
from tuiuiu.tuiuiusearch.backends.db import DatabaseSearchBackend
from tuiuiu.tuiuiusearch.management.commands.update_index import group_models_by_index


class BackendTests(TuiuiuTestUtils):
    ...
    def setUp(self):
        ...
        for backend_name, backend_conf in settings.TUIUIUSEARCH_BACKENDS.items():
            ...
            raise unittest.SkipTest("No TUIUIUSEARCH_BACKENDS entry for the backend %s" % self.backend_path)
...
@override_settings(
    TUIUIUSEARCH_BACKENDS={
        'default': {'BACKEND': 'tuiuiu.tuiuiusearch.backends.db'}
    }
)
class TestBackendLoader(TestCase):
    ...
    def test_import_by_path(self):
        db = get_search_backend(backend='tuiuiu.tuiuiusearch.backends.db')
        self.assertIsInstance(db, DatabaseSearchBackend)

    def test_import_by_full_path(self):
        db = get_search_backend(backend='tuiuiu.tuiuiusearch.backends.db.DatabaseSearchBackend')
        self.assertIsInstance(db, DatabaseSearchBackend)

    def test_nonexistent_backend_import(self):
        self.assertRaises(
            InvalidSearchBackendError, get_search_backend, backend='tuiuiu.tuiuiusearch.backends.doesntexist'
        )
