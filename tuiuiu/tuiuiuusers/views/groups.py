...
from tuiuiu.tuiuiuusers.forms import GroupForm, GroupPagePermissionFormSet
...
def index(request):
    ...
    if request.is_ajax():
        return render(request, "tuiuiuusers/groups/results.html", {
            'groups': groups,
        ...
        return render(request, "tuiuiuusers/groups/index.html", {
            'search_form': form,
            'groups': groups,
            'is_searching': is_searching,
            'ordering': ordering,
...
def create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and all(panel.is_valid() for panel in permission_panels):
            ...
            messages.success(request, _("Group '{0}' created.").format(group), buttons=[
                messages.button(reverse('tuiuiuusers_groups:edit', args=(group.id,)), _('Edit'))
            ])
    ...
    return render(request, 'tuiuiuusers/groups/create.html', {
        'form': form,
