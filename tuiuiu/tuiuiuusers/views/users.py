...
def get_user_creation_form():
    form_setting = 'TUIUIU_USER_CREATION_FORM'
...
def get_user_edit_form():
    form_setting = 'TUIUIU_USER_EDIT_FORM'
    if hasattr(settings, form_setting):
        return get_custom_user_form(form_setting)
    else:
        return UserEditForm
...
def index(request):
    ...
    if request.is_ajax():
        return render(request, "tuiuiuusers/users/results.html", {
            'users': users,
        ...
        return render(request, "tuiuiuusers/users/index.html", {
            'search_form': form,
            'users': users,
            'is_searching': is_searching,
            'ordering': ordering,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, _("User '{0}' created.").format(user), buttons=[
                messages.button(reverse('tuiuiuusers_users:edit', args=(user.pk,)), _('Edit'))
            ])
