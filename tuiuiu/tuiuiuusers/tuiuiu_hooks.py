...
from tuiuiu.tuiuiuusers.urls import groups, users
from tuiuiu.tuiuiuusers.utils import user_can_delete_user
from tuiuiu.tuiuiuusers.widgets import UserListingButton


@hooks.register('register_admin_urls')
def register_admin_urls():
    return [
        url(r'^users/', include(users, app_name='tuiuiuusers_users', namespace='tuiuiuusers_users')),
        url(r'^groups/', include(groups, app_name='tuiuiuusers_groups', namespace='tuiuiuusers_groups')),
    ]
...
def register_users_menu_item():
    return UsersMenuItem(
        _('Users'),
        urlresolvers.reverse('tuiuiuusers_users:index'),
...
def register_groups_menu_item():
    return GroupsMenuItem(
        _('Groups'),
        urlresolvers.reverse('tuiuiuusers_groups:index'),
...
def register_users_search_area():
    return UsersSearchArea(
        _('Users'), urlresolvers.reverse('tuiuiuusers_users:index'),
        name='users',
        classnames='icon icon-user',
        order=600)
...
def user_listing_buttons(context, user):
    yield UserListingButton(_('Edit'), reverse('tuiuiuusers_users:edit', args=[user.pk]), attrs={'title': _('Edit this user')}, priority=10)
    if user_can_delete_user(context.request.user, user):
        yield UserListingButton(_('Delete'), reverse('tuiuiuusers_users:delete', args=[user.pk]), classes={'no'}, attrs={'title': _('Delete this user')}, priority=20)
