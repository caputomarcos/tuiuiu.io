...
from tuiuiu.tuiuiucore.compat import AUTH_USER_APP_LABEL, AUTH_USER_MODEL_NAME
from tuiuiu.tuiuiucore.models import (
    Collection, GroupCollectionPermission, GroupPagePermission, Page)
from tuiuiu.tuiuiuusers.forms import UserCreationForm, UserEditForm
from tuiuiu.tuiuiuusers.models import UserProfile
from tuiuiu.tuiuiuusers.views.users import get_user_creation_form, get_user_edit_form
...
class TestUserFormHelpers(TestCase):
    ...
    @override_settings(
        TUIUIU_USER_CREATION_FORM='tuiuiu.tuiuiuusers.tests.CustomUserCreationForm'
    )
    ...
    @override_settings(
        TUIUIU_USER_EDIT_FORM='tuiuiu.tuiuiuusers.tests.CustomUserEditForm'
    )
...
class TestUserIndexView(TestCase, TuiuiuTestUtils):
    ...
    def get(self, params={}):
        return self.client.get(reverse('tuiuiuusers_users:index'), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuusers/users/index.html')
        self.assertContains(response, 'testuser')

    def test_allows_negative_ids(self):
        # see https://github.com/tuiuiu/tuiuiu/issues/565
...
class TestUserCreateView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiuusers_users:add'), params)

    def post(self, post_data={}):
        return self.client.post(reverse('tuiuiuusers_users:add'), post_data)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuusers/users/create.html')
    ...
    def test_create(self):
        ...
        self.assertRedirects(response, reverse('tuiuiuusers_users:index'))
    ...
    @override_settings(
        TUIUIU_USER_CREATION_FORM='tuiuiu.tuiuiuusers.tests.CustomUserCreationForm',
        TUIUIU_USER_CUSTOM_FIELDS=['country', 'document'],
    )
...
class TestUserDeleteView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # create a user that should be visible in the listing
        self.test_user = get_user_model().objects.create_user(
            username='testuser',
    ...
    def get(self, params={}):
        return self.client.get(reverse('tuiuiuusers_users:delete', args=(self.test_user.pk,)), params)

    def post(self, post_data={}):
        return self.client.post(reverse('tuiuiuusers_users:delete', args=(self.test_user.pk,)), post_data)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuusers/users/confirm_delete.html')
    ...
    def test_user_cannot_delete_self(self):
        response = self.client.get(reverse('tuiuiuusers_users:delete', args=(self.current_user.pk,)))
    ...
    def test_user_can_delete_other_superuser(self):
        response = self.client.get(reverse('tuiuiuusers_users:delete', args=(self.superuser.pk,)))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuusers/users/confirm_delete.html')

        response = self.client.post(reverse('tuiuiuusers_users:delete', args=(self.superuser.pk,)))
        # Should redirect back to index
        self.assertRedirects(response, reverse('tuiuiuusers_users:index'))
...
class TestUserDeleteViewForNonSuperuser(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # create a user that should be visible in the listing
        self.test_user = get_user_model().objects.create_user(
            username='testuser',
    ...
    def test_simple(self):
        response = self.client.get(reverse('tuiuiuusers_users:delete', args=(self.test_user.pk,)))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuusers/users/confirm_delete.html')

    def test_delete(self):
        response = self.client.post(reverse('tuiuiuusers_users:delete', args=(self.test_user.pk,)))
    ...
    def test_user_cannot_delete_self(self):
        response = self.client.post(reverse('tuiuiuusers_users:delete', args=(self.deleter_user.pk,)))
...
class TestUserEditView(TestCase, TuiuiuTestUtils):
    def setUp(self):
    ...
    def get(self, params={}, user_id=None):
        return self.client.get(reverse('tuiuiuusers_users:edit', args=(user_id or self.test_user.pk, )), params)

    def post(self, post_data={}, user_id=None):
        return self.client.post(reverse('tuiuiuusers_users:edit', args=(user_id or self.test_user.pk, )), post_data)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuusers/users/edit.html')
...
class TestUserProfileCreation(TestCase, TuiuiuTestUtils):
    def setUp(self):
        # Create a user
        self.test_user = get_user_model().objects.create_user(
            username='testuser',
    ...
    def test_user_created_without_profile(self):
        ...
        with self.assertRaises(UserProfile.DoesNotExist):
            self.test_user.tuiuiu_userprofile
...
class TestGroupIndexView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiuusers_groups:index'), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuusers/groups/index.html')
...
class TestGroupCreateView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(reverse('tuiuiuusers_groups:add'), params)
    ...
    def post(self, post_data={}):
        ...
        return self.client.post(reverse('tuiuiuusers_groups:add'), post_data)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuusers/groups/create.html')
...
class TestGroupEditView(TestCase, TuiuiuTestUtils):
    def setUp(self):
    ...
    def get(self, params={}, group_id=None):
        return self.client.get(reverse('tuiuiuusers_groups:edit', args=(group_id or self.test_group.pk, )), params)
    ...
    def post(self, post_data={}, group_id=None):
        ...
        return self.client.post(reverse(
            'tuiuiuusers_groups:edit', args=(group_id or self.test_group.pk, )), post_data)

    def add_non_registered_perm(self):
        # Some groups may have django permissions assigned that are not
        # hook-registered as part of the tuiuiu interface. We need to ensure
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiuusers/groups/edit.html')
    ...
    def test_group_edit_adding_document_permissions_same_collection(self):
        ...
        self.assertEqual(
            self.test_group.collection_permissions.filter(
                permission__content_type__app_label='tuiuiudocs'
            ).count(),
            1
        )
        response = self.post({
