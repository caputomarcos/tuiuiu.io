...
from tuiuiu.tests.snippets.forms import FancySnippetForm
from tuiuiu.tests.snippets.models import (
...
from tuiuiu.tests.testapp.models import Advert, AdvertWithTabbedInterface, SnippetChooserModel
from tuiuiu.tests.utils import TuiuiuTestUtils
from tuiuiu.tuiuiuadmin.forms import TuiuiuAdminModelForm
from tuiuiu.tuiuiucore.models import Page
from tuiuiu.tuiuiusnippets.blocks import SnippetChooserBlock
from tuiuiu.tuiuiusnippets.edit_handlers import SnippetChooserPanel
from tuiuiu.tuiuiusnippets.models import SNIPPET_MODELS, register_snippet
from tuiuiu.tuiuiusnippets.views.snippets import get_snippet_edit_handler


class TestSnippetIndexView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiusnippets:index'), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiusnippets/snippets/index.html')

    def test_displays_snippet(self):
        self.assertContains(self.get(), "Adverts")


class TestSnippetListView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse('tuiuiusnippets:list',
                                       args=('tests', 'advert')),
                               params)
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiusnippets/snippets/type_index.html')
...
class TestModelOrdering(TestCase, TuiuiuTestUtils):
    def setUp(self):
        for i in range(1, 10):
    ...
    def test_listing_respects_model_ordering(self):
        response = self.client.get(
            reverse('tuiuiusnippets:list', args=('tests', 'advertwithtabbedinterface'))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_chooser_respects_model_ordering(self):
        response = self.client.get(
            reverse('tuiuiusnippets:choose', args=('tests', 'advertwithtabbedinterface'))
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['items'][0].text, "aaaadvert")


class TestSnippetListViewWithSearchableSnippet(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
...
class TestSnippetCreateView(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}, model=Advert):
        args = (model._meta.app_label, model._meta.model_name)
        return self.client.get(reverse('tuiuiusnippets:add', args=args), params)

    def post(self, post_data={}, model=Advert):
        args = (model._meta.app_label, model._meta.model_name)
        return self.client.post(reverse('tuiuiusnippets:add', args=args), post_data)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiusnippets/snippets/create.html')
    ...
    def test_snippet_with_tabbed_interface(self):
        response = self.client.get(reverse('tuiuiusnippets:add',
    ...
    def test_create(self):
        ...
        self.assertRedirects(response, reverse('tuiuiusnippets:list', args=('tests', 'advert')))
    ...
    def test_create_with_tags(self):
        ...
        self.assertRedirects(response, reverse('tuiuiusnippets:list',
                                               args=('tests', 'advert')))
...
class BaseTestSnippetEditView(TestCase, TuiuiuTestUtils):

    def get(self, params={}):
        snippet = self.test_snippet
        args = (snippet._meta.app_label, snippet._meta.model_name, snippet.id)
        return self.client.get(reverse('tuiuiusnippets:edit', args=args), params)

    def post(self, post_data={}):
        snippet = self.test_snippet
        args = (snippet._meta.app_label, snippet._meta.model_name, snippet.id)
        return self.client.post(reverse('tuiuiusnippets:edit', args=args), post_data)
...
class TestSnippetEditView(BaseTestSnippetEditView):
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiusnippets/snippets/edit.html')
        self.assertNotContains(response, '<ul class="tab-nav merged">')
    ...
    def test_non_existant_model(self):
        response = self.client.get(reverse('tuiuiusnippets:edit', args=('tests', 'foo', self.test_snippet.id)))
        self.assertEqual(response.status_code, 404)

    def test_nonexistant_id(self):
        response = self.client.get(reverse('tuiuiusnippets:edit', args=('tests', 'advert', 999999)))
        self.assertEqual(response.status_code, 404)
...
class TestSnippetDelete(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
    ...
    def test_delete_get(self):
        response = self.client.get(reverse('tuiuiusnippets:delete', args=('tests', 'advert', self.test_snippet.id, )))
        self.assertEqual(response.status_code, 200)

    def test_delete_post(self):
        response = self.client.post(
            reverse('tuiuiusnippets:delete', args=('tests', 'advert', self.test_snippet.id, ))
        )
...
class TestSnippetChooserPanel(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
...
class TestSnippetChoose(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
    ...
    def get(self, params=None):
        return self.client.get(reverse('tuiuiusnippets:choose',
                                       args=('tests', 'advert')),
                               params or {})

    def test_simple(self):
        response = self.get()
        self.assertTemplateUsed(response, 'tuiuiusnippets/chooser/choose.html')
...
class TestSnippetChooseWithSearchableSnippet(TestCase, TuiuiuTestUtils):
    def setUp(self):
        self.login()
...
class TestSnippetChosen(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
    ...
    def get(self, pk, params=None):
        return self.client.get(reverse('tuiuiusnippets:chosen',
                                       args=('tests', 'advert', pk)),
                               params or {})

    def test_choose_a_page(self):
        response = self.get(pk=Advert.objects.all()[0].pk)
        self.assertTemplateUsed(response, 'tuiuiusnippets/chooser/chosen.js')
...
class TestAddOnlyPermissions(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
    ...
    def test_get_index(self):
        response = self.client.get(reverse('tuiuiusnippets:list',
                                   args=('tests', 'advert')))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiusnippets/snippets/type_index.html')
    ...
    def test_get_edit(self):
        response = self.client.get(reverse('tuiuiusnippets:edit',
...
class TestDeleteOnlyPermissions(TestCase, TuiuiuTestUtils):
    fixtures = ['test.json']
    ...
    def test_get_delete(self):
        response = self.client.get(reverse('tuiuiusnippets:delete', args=('tests', 'advert', self.test_snippet.id, )))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tuiuiusnippets/snippets/confirm_delete.html')


class TestSnippetEditHandlers(TestCase, TuiuiuTestUtils):
    def test_standard_edit_handler(self):
        ...
        self.assertTrue(issubclass(form_class, TuiuiuAdminModelForm))
...
class TestInlinePanelMedia(TestCase, TuiuiuTestUtils):
    ...
    def test_inline_panel_media(self):
        ...
        response = self.client.get(reverse('tuiuiusnippets:add', args=('snippetstests', 'multisectionrichtextsnippet')))
        self.assertEqual(response.status_code, 200)
