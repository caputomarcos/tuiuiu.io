...
from tuiuiu.contrib.settings.registry import SettingMenuItem
from tuiuiu.contrib.settings.views import get_setting_edit_handler
...
class TestSettingMenu(TestCase, TuiuiuTestUtils):
    ...
    def login_only_admin(self):
        ...
        user.user_permissions.add(Permission.objects.get_by_natural_key(
            codename='access_admin', app_label='tuiuiuadmin', model='admin'))
    ...
    def test_menu_item_in_admin(self):
        ...
        self.assertContains(response, reverse('settings:edit', args=('tests', 'testsetting')))
    ...
    def test_menu_item_no_permissions(self):
        ...
        self.assertNotContains(response, reverse('settings:edit', args=('tests', 'testsetting')))
...
class BaseTestSettingView(TestCase, TuiuiuTestUtils):
    ...
    def edit_url(self, setting, site_pk=1):
        ...
        return reverse('settings:edit', args=args)
...
class TestSettingEditView(BaseTestSettingView):
    ...
    def test_non_existant_model(self):
        response = self.client.get(reverse('settings:edit', args=['test', 'foo', 1]))
...
class TestMultiSite(BaseTestSettingView):
    ...
    def test_redirect_to_default(self):
        ...
        """
        start_url = reverse('settings:edit', args=[
            'tests', 'testsetting'])
        dest_url = 'http://testserver' + reverse('settings:edit', args=[
    ...
    def test_redirect_to_current(self):
        ...
        start_url = reverse('settings:edit', args=[
            'tests', 'testsetting'])
        dest_url = 'http://example.com' + reverse('settings:edit', args=[
...
class TestAdminPermission(TestCase, TuiuiuTestUtils):
