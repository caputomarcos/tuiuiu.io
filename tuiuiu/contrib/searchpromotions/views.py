...
from tuiuiu.contrib.searchpromotions import forms
...
from tuiuiu.tuiuiuadmin.utils import any_permission_required, permission_required
from tuiuiu.tuiuiusearch import forms as search_forms
from tuiuiu.tuiuiusearch.models import Query


@any_permission_required(
    'searchpromotions.add_searchpromotion',
    'searchpromotions.change_searchpromotion',
    'searchpromotions.delete_searchpromotion'
)
...
def index(request):
    ...
    if request.is_ajax():
        return render(request, "searchpromotions/results.html", {
            'is_searching': is_searching,
            'queries': queries,
            'query_string': query_string,
        })
    else:
        return render(request, 'searchpromotions/index.html', {
            'is_searching': is_searching,
            'queries': queries,
            'query_string': query_string,
...
@permission_required('searchpromotions.add_searchpromotion')
def add(request):
        ...
        if query_form.is_valid():
            ...
            if save_searchpicks(query, query, searchpicks_formset):
                messages.success(request, _("Editor's picks for '{0}' created.").format(query), buttons=[
                    messages.button(reverse('searchpromotions:edit', args=(query.id,)), _('Edit'))
                ])
...
@permission_required('searchpromotions.change_searchpromotion')
def edit(request, query_id):
    ...
    return render(request, 'searchpromotions/edit.html', {
        'query_form': query_form,
        'searchpicks_formset': searchpicks_formset,
        'query': query,
    })
...
@permission_required('searchpromotions.delete_searchpromotion')
