...
from tuiuiu.tuiuiuadmin.edit_handlers import FieldPanel, PageChooserPanel, InlinePanel
...
def get_backend():
    global BACKEND
    if BACKEND is None:
        backend_name = getattr(settings, 'TUIUIU_EXPERIMENTS_BACKEND', 'tuiuiu.contrib.experiments.backends.db')
...
class Experiment(ClusterableModel):
    ...
    control_page = models.ForeignKey('tuiuiucore.Page', related_name='+', on_delete=models.CASCADE)
    goal = models.ForeignKey('tuiuiucore.Page', related_name='+', on_delete=models.SET_NULL, null=True, blank=True)
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default='draft')
    winning_variation = models.ForeignKey('tuiuiucore.Page', related_name='+', on_delete=models.SET_NULL, null=True)
...
class Alternative(Orderable):
    ...
    page = models.ForeignKey('tuiuiucore.Page', related_name='+', on_delete=models.CASCADE)
...
class ExperimentHistory(models.Model):
    ...
    variation = models.ForeignKey('tuiuiucore.Page', related_name='+', on_delete=models.CASCADE)
