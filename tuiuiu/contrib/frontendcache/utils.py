...
logger = logging.getLogger('tuiuiu.frontendcache')
...
def get_backends(backend_settings=None, backends=None):
    # Get backend settings from FRONTENDCACHE setting
    if backend_settings is None:
        backend_settings = getattr(settings, 'FRONTENDCACHE', None)

    # Fallback to using FRONTENDCACHE_LOCATION setting (backwards compatibility)
    if backend_settings is None:
        cache_location = getattr(settings, 'FRONTENDCACHE_LOCATION', None)
