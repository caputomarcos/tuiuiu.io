...
class assets_mixin(object):
    ...
    def bump_client_version(self):
        """
        Writes the current Tuiuiu version number into package.json
        """
...
class check_bdist_egg(bdist_egg):
    ...
    sentinel_dir = 'tuiuiu/tuiuiuadmin/static/'
    ...
    def run(self):
        ...
        if not os.path.isdir(self.sentinel_dir):
            print("\n".join([  # noqa
                "************************************************************",
                "The front end assets for Tuiuiu are missing.",
