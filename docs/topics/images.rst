...
.. glossary::
    ...
    ``fill``
        ...
        **Cropping closer to the focal point**
        ...
        By default, Tuiuiu will only crop enough to change the aspect ratio of the image to match the ratio in the resize-rule.
...
.. Note::
    Tuiuiu does not allow deforming or stretching images. Image dimension ratios will always be kept. Tuiuiu also *does not support upscaling*. Small images forced to appear at larger sizes will "max out" at their native dimensions.
...
Tuiuiu provides two shortcuts to give greater control over the ``img`` element:
...
Tuiuiu can assign the image data to another variable using Django's ``as`` syntax:
...
Tuiuiu comes with three pre-defined image formats, but more can be defined in Python by the developer. These formats are:
...
Tuiuiu may automatically change the format of some images when they are resized:
...
Tuiuiu's JPEG image quality setting defaults to 85 (which is quite high). This
can be changed either globally or on a per-tag basis.

Changing globally
^^^^^^^^^^^^^^^^^

Use the ``TUIUIUIMAGES_JPEG_QUALITY`` setting to change the global default JPEG
quality:
...
.. code-block:: python
    ...
    >>> from tuiuiu.tuiuiuimages.models import Rendition
