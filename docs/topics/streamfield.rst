...
.. code-block:: python
    ...
    from tuiuiu.tuiuiucore.fields import StreamField
    from tuiuiu.tuiuiucore import blocks
    from tuiuiu.tuiuiuadmin.edit_handlers import FieldPanel, StreamFieldPanel
    from tuiuiu.tuiuiuimages.blocks import ImageChooserBlock
...
``icon``
  The name of the icon to display for this block type in the menu of available block types. For a list of icon names, see the Tuiuiu style guide, which can be enabled by adding ``tuiuiu.contrib.tuiuiustyleguide`` to your project's ``INSTALLED_APPS``.
...
The basic block types provided by Tuiuiu are as follows:

CharBlock
~~~~~~~~~

``tuiuiu.tuiuiucore.blocks.CharBlock``
...
``tuiuiu.tuiuiucore.blocks.TextBlock``
...
``tuiuiu.tuiuiucore.blocks.EmailBlock``
...
``tuiuiu.tuiuiucore.blocks.IntegerBlock``
...
``tuiuiu.tuiuiucore.blocks.FloatBlock``
...
``tuiuiu.tuiuiucore.blocks.DecimalBlock``
...
``tuiuiu.tuiuiucore.blocks.RegexBlock``
...
``tuiuiu.tuiuiucore.blocks.URLBlock``
...
``tuiuiu.tuiuiucore.blocks.BooleanBlock``
...
``tuiuiu.tuiuiucore.blocks.DateBlock``

A date picker. The keyword arguments ``required``, ``help_text`` and ``format`` are accepted.

``format`` (default: None)
  Date format. This must be one of the recognised formats listed in the `DATE_INPUT_FORMATS <https://docs.djangoproject.com/en/1.10/ref/settings/#std:setting-DATE_INPUT_FORMATS>`_ setting. If not specifed Tuiuiu will use ``TUIUIU_DATE_FORMAT`` setting with fallback to '%Y-%m-%d'.

TimeBlock
~~~~~~~~~

``tuiuiu.tuiuiucore.blocks.TimeBlock``
...
``tuiuiu.tuiuiucore.blocks.DateTimeBlock``

A combined date / time picker. The keyword arguments ``required``, ``help_text`` and ``format`` are accepted.

``format`` (default: None)
  Date format. This must be one of the recognised formats listed in the `DATETIME_INPUT_FORMATS <https://docs.djangoproject.com/en/1.10/ref/settings/#std:setting-DATETIME_INPUT_FORMATS>`_ setting. If not specifed Tuiuiu will use ``TUIUIU_DATETIME_FORMAT`` setting with fallback to '%Y-%m-%d %H:%M'.

RichTextBlock
~~~~~~~~~~~~~

``tuiuiu.tuiuiucore.blocks.RichTextBlock``
...
``tuiuiu.tuiuiucore.blocks.RawHTMLBlock``
...
``tuiuiu.tuiuiucore.blocks.BlockQuoteBlock``
...
``tuiuiu.tuiuiucore.blocks.ChoiceBlock``
...
``tuiuiu.tuiuiucore.blocks.PageChooserBlock``

A control for selecting a page object, using Tuiuiu's page browser. The following keyword arguments are accepted:
...
``tuiuiu.tuiuiudocs.blocks.DocumentChooserBlock``
...
``tuiuiu.tuiuiuimages.blocks.ImageChooserBlock``
...
``tuiuiu.tuiuiusnippets.blocks.SnippetChooserBlock``
...
``tuiuiu.tuiuiuembeds.blocks.EmbedBlock``
...
``tuiuiu.tuiuiucore.blocks.StaticBlock``
...
``tuiuiu.tuiuiucore.blocks.StructBlock``
...
``tuiuiu.tuiuiucore.blocks.ListBlock``
...
``tuiuiu.tuiuiucore.blocks.StreamBlock``
...
.. code-block:: html+django
    ...
    {% load tuiuiucore_tags tuiuiuimages_tags %}
...
This limitation does not apply to StructBlock and StreamBlock values as children of a StructBlock, because Tuiuiu implements these as complex objects that know their own template rendering, even when not wrapped in a ``BoundBlock``. For example, if a StructBlock is nested in another StructBlock, as in:
...
If you need to implement a custom UI, or handle a datatype that is not provided by Tuiuiu's built-in block types (and cannot built up as a structure of existing fields), it is possible to define your own custom block types. For further guidance, refer to the source code of Tuiuiu's built-in block classes.

For block types that simply wrap an existing Django form field, Tuiuiu provides an abstract class ``tuiuiu.tuiuiucore.blocks.FieldBlock`` as a helper. Subclasses just need to set a ``field`` property that returns the form field object:
...
.. code-block:: python
    ...
    from tuiuiu.tuiuiucore.rich_text import RichText
