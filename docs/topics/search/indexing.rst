...
If you have created some extra fields in a subclass of Page or Image, you may want to add these new fields to the search index too so that a user's search query will match on their content. See :ref:`tuiuiusearch_indexing_fields` for info on how to do this.

If you have a custom model that you would like to make searchable, see :ref:`tuiuiusearch_indexing_models`.


.. _tuiuiusearch_indexing_update:
...
``tuiuiusearch`` provides some signal handlers which bind to the save/delete signals of all indexed models. This would automatically add and delete them from all backends you have registered in ``TUIUIUSEARCH_BACKENDS``. These signal handlers are automatically registered when the ``tuiuiu.tuiuiusearch`` app is loaded.


The ``update_index`` command
----------------------------

Tuiuiu also provides a command for rebuilding the index from scratch.
...
```````
 ...
 - **es_extra** (``dict``) - This field is to allow the developer to set or override any setting on the field in the ElasticSearch mapping. Use this if you want to make use of any ElasticSearch features that are not yet supported in Tuiuiu.
...
.. _tuiuiusearch_index_relatedfields:
...
.. topic:: Filtering on ``index.RelatedFields``
    ...
    Filtering on ``index.RelatedFields`` with the ``QuerySet`` API is planned for a future release of Tuiuiu.

.. _tuiuiusearch_indexing_callable_fields:
...
---------------------------------------
 ...
 .. note::
     ...
     This is not supported in the :ref:`tuiuiusearch_backends_database`
