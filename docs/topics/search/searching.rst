...
.. _tuiuiusearch_searching_pages:

Searching QuerySets
===================

Tuiuiu search is built on Django's `QuerySet API <https://docs.djangoproject.com/en/1.8/ref/models/querysets/>`_. You should be able to search any Django QuerySet provided the model and the fields being filtered on have been added to the search index.


Searching Pages
---------------

Tuiuiu provides a shortcut for searching pages: the ``.search()`` ``QuerySet`` method. You can call this on any ``PageQuerySet``. For example:

.. code-block:: python

    # Search future EventPages
    >>> from tuiuiu.tuiuiucore.models import EventPage
...
.. note::
    ...
    The ``search()`` method will convert your ``QuerySet`` into an instance of one of Tuiuiu's ``SearchResults`` classes (depending on backend). This means that you must perform filtering before calling ``search()``.


.. _tuiuiusearch_images_documents_custom_models:

Searching Images, Documents and custom models
---------------------------------------------

Tuiuiu's document and image models provide a ``search`` method on their QuerySets, just as pages do:
...
:ref:`Custom models <tuiuiusearch_indexing_models>` can be searched by using the ``search`` method on the search backend directly:
...
.. _tuiuiusearch_specifying_fields:

Specifying the fields to search
-------------------------------

By default, Tuiuiu will search all fields that have been indexed using ``index.SearchField``.
...
.. _tuiuiusearch_annotating_results_with_score:
...
.. code-block:: python
    ...
    def search(request):
        ...
        if search_query:
            ...
            # Log the query so Tuiuiu can suggest promoted results
...
This functionality is provided by the :mod:`~tuiuiu.contrib.tuiuiusearchpromotions` contrib module.
