...
in how your model and its objects are represented in Tuiuiu's CMS. This page
...
---------------------------------------------------------
Tuiuiu's ``ModelAdmin`` class isn't the same as Django's
---------------------------------------------------------

Tuiuiu's ``ModelAdmin`` class is designed to be used in a similar way to
...
In Tuiuiu, controlling which fields appear in add/edit forms for your
...
just a standard Django ``Model``. Because of this, Tuiuiu's ``ModelAdmin``
...
extends ``tuiuiucore.models.Page``) or not.

Pages in Tuiuiu have some unique properties, and require additional views,
...
Tuiuiu provides its own specific views for managing any custom page types
...
simply redirects users to Tuiuiu's existing page management views wherever
...
Tuiuiu's ``ModelAdmin`` class is 'modular'
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Unlike Django's class of the same name, tuiuiuadmin's ``ModelAmin`` acts
...
Tuiuiu will automatically prepended paths for each path with ``STATIC_URL``,
...
For all modeladmin views, Tuiuiu looks for templates in the following folders
...
.. code-block:: python
    ...
    from tuiuiu.contrib.modeladmin.views import IndexView
    from tuiuiu.contrib.modeladmin.options import ModelAdmin
...
'helper' classes) and can be found in ``tuiuiu.contrib.modeladmin.helpers``.
...
when your model extends ``tuiuiucore.models.Page``, otherwise
...
``PageAdminURLHelper`` (if your  model extend's Tuiuiu's ``Page`` model), and
...
.. code-block:: python
    ...
    from tuiuiu.contrib.modeladmin.helpers import AdminURLHelper
    from tuiuiu.contrib.modeladmin.options import ModelAdmin, modeladmin_register
    from .models import MyModel
...
class is used when your model extends ``tuiuiucore.models.Page``,
otherwise ``modeladmin.helpers.permission.PermissionHelper`` is used.

If you find that the above helper classes don't cater for your needs, you can
easily create your own helper class, by sub-classing
``PermissionHelper`` or (if your  model extend's Tuiuiu's ``Page`` model)
...
.. code-block:: python
    ...
    from tuiuiu.contrib.modeladmin.helpers import PermissionHelper
    from tuiuiu.contrib.modeladmin.options import ModelAdmin, modeladmin_register
    from .models import MyModel
...
your  model extend's Tuiuiu's ``Page`` model) ``PageButtonHelper``, and
...
.. code-block:: python
    ...
    from tuiuiu.contrib.modeladmin.helpers import ButtonHelper
    from tuiuiu.contrib.modeladmin.options import ModelAdmin, modeladmin_register
    from .models import MyModel
