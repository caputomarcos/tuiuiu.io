...
pages for any model in your Tuiuiu project, and add navigation elements to the
Tuiuiu admin area so that you can access them. Simply extend the
``ModelAdmin`` class, override a few attributes to suit your needs, register
it with Tuiuiu using an easy one-line ``modeladmin_register`` method
...
existing admin functionality that Tuiuiu provides.
...
- A customisable list view, allowing you to control what values are displayed
  ...
  more.
- Access your list views from the Tuiuiu admin menu easily with automatically
...
- Simple, robust **add** and **edit** views for your non-Page models that use
  the panel configurations defined on your model using Tuiuiu's edit panels.
- For Page models, the system directs to Tuiuiu's existing add and
  edit views, and returns you back to the correct list page, for a seamless
  experience.
- Full respect for permissions assigned to your Tuiuiu users and groups. Users
  will only be able to do what you want them to!
- All you need to easily hook your ``ModelAdmin`` classes into Tuiuiu, taking
...
Add ``tuiuiu.contrib.modeladmin`` to your ``INSTALLED_APPS``:
...
model, with a menu item added to the menu in the Tuiuiu admin area so that you
can get to it.

``tuiuiu_hooks.py`` in your app directory would look something like this:

.. code-block:: python

    from tuiuiu.contrib.modeladmin.options import (
    ...
    class MyPageModelAdmin(ModelAdmin):
        ...
        exclude_from_explorer = False # or True to exclude pages of this type from Tuiuiu's explorer view
        list_display = ('title', 'example_field2', 'example_field3', 'live')
        list_filter = ('live', 'example_field2', 'example_field3')
        search_fields = ('title',)

    # Now you just need to register your customised ModelAdmin class with Tuiuiu
...
Tuiuiu's admin menu. Some of the models might extend Page, and others might
...
Registering multiple classes in one ``tuiuiu_hooks.py`` file
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
