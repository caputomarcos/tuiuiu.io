...
-   If the value of a field is ``None``, an empty string, or an iterable
    without elements, Tuiuiu will display a dash (-) for that column. You can
    ...
    The above will tell Tuiuiu to order by the ``first_name`` field when
...
Searching is all handled via Django's queryset API, rather than using Tuiuiu's
...
----------------------------------------------------
``tuiuiu.contrib.modeladmin.mixins.ThumbnailMixin``
----------------------------------------------------

If you're using ``tuiuiuimages.Image`` to define an image for each item in
...
.. code-block:: python
    ...
    from django.db import models
    from tuiuiu.contrib.modeladmin.mixins import ThumbnailMixin
    from tuiuiu.contrib.modeladmin.options import ModelAdmin

    class Person(models.Model):
        name = models.CharField(max_length=255)
        avatar = models.ForeignKey('tuiuiuimages.Image', on_delete=models.SET_NULL, null=True)
        likes_cat_gifs = models.NullBooleanField()
