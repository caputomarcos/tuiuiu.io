...
.. automodule:: tuiuiu.tuiuiucore.models

This document contains reference information for the model classes inside the ``tuiuiucore`` module.
...
.. class:: Page
    ...
    .. attribute:: show_in_menus
        ...
        This is used by the :meth:`~tuiuiu.tuiuiucore.query.PageQuerySet.in_menu` QuerySet filter.
...
.. class:: Page
    ...
    .. attribute:: search_fields
        ...
        A list of fields to be indexed by the search engine. See Search docs :ref:`tuiuiusearch_indexing_fields`
    ...
    .. attribute:: is_creatable
        ...
        Controls if this page can be created through the Tuiuiu administration. Defaults to True, and is not inherited by subclasses. This is useful when using `multi-table inheritance <https://docs.djangoproject.com/en/1.8/topics/db/models/#multi-table-inheritance>`_, to stop the base model from being created as an actual page.

    .. attribute:: base_form_class

        The form class used as a base for editing Pages of this type in the Tuiuiu page editor.
        This attribute can be set on a model to customise the Page editor form.
        Forms must be a subclass of :class:`~tuiuiu.tuiuiuadmin.forms.TuiuiuAdminPageForm`.
...
This configuration is used by the :class:`~tuiuiu.tuiuiucore.middleware.SiteMiddleware` middleware class which checks each request against this configuration and appends the Site object to the Django request object.
...
.. class:: Site
    ...
    .. attribute:: site_name
        ...
        A human-readable name for the site. This is not used by Tuiuiu itself, but is suitable for use on the site front-end, such as in ``<title>`` elements.

        For example: ``Rod's World of Birds``

    .. attribute:: root_page

        (foreign key to :class:`~tuiuiu.tuiuiucore.models.Page`)
...
Every time a page is edited a new ``PageRevision`` is created and saved to the database. It can be used to find the full history of all changes that have been made to a page and it also provides a place for new changes to be kept before going live.
 ...
 - Revisions can be created from any :class:`~tuiuiu.tuiuiucore.models.Page` object by calling its :meth:`~Page.save_revision` method
 - The content of the page is JSON-serialised and stored in the :attr:`~PageRevision.content_json` field
 - You can retrieve a ``PageRevision`` as a :class:`~tuiuiu.tuiuiucore.models.Page` object by calling the :meth:`~PageRevision.as_page_object` method
...
.. class:: PageRevision
    ...
    .. automethod:: as_page_object
        ...
        This method retrieves this revision as an instance of its :class:`~tuiuiu.tuiuiucore.models.Page` subclass.
