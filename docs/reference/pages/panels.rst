...
.. module:: tuiuiu.tuiuiuadmin.edit_handlers
...
.. class:: FieldPanel(field_name, classname=None, widget=None)
    ...
    .. attribute:: FieldPanel.classname
        ...
        The CSS class ``full`` can be used to format the panel so it covers the full width of the Tuiuiu page editor.
...
.. class:: MultiFieldPanel(children, heading="", classname=None)
    ...
    This panel condenses several :class:`~tuiuiu.tuiuiuadmin.edit_handlers.FieldPanel` s or choosers, from a ``list`` or ``tuple``, under a single ``heading`` string.
...
.. class:: PageChooserPanel(field_name, page_type=None, can_choose_root=False)
    ...
    You can explicitly link :class:`~tuiuiu.tuiuiucore.models.Page`-derived models together using the :class:`~tuiuiu.tuiuiucore.models.Page` model and ``PageChooserPanel``.

    .. code-block:: python

        from tuiuiu.tuiuiucore.models import Page
        from tuiuiu.tuiuiuadmin.edit_handlers import PageChooserPanel
...
.. module:: tuiuiu.tuiuiuimages.edit_handlers

.. class:: ImageChooserPanel(field_name)

    Tuiuiu includes a unified image library, which you can access in your models through the :class:`~tuiuiu.tuiuiuimages.models.Image` model and the ``ImageChooserPanel`` chooser. Here's how:
...
.. module:: tuiuiu.tuiuiuforms.edit_handlers

.. class:: FormSubmissionsPanel

    This panel adds a single, read-only section in the edit interface for pages implementing the :class:`~tuiuiu.tuiuiuforms.models.AbstractForm` model.
    It includes the number of total submissions for the given form and also a link to the listing of submissions.

    .. code-block:: python

        from tuiuiu.tuiuiuforms.models import AbstractForm
        from tuiuiu.tuiuiuforms.edit_handlers import FormSubmissionsPanel
...
.. module:: tuiuiu.tuiuiudocs.edit_handlers

.. class:: DocumentChooserPanel(field_name)

    For files in other formats, Tuiuiu provides a generic file store through the :class:`~tuiuiu.tuiuiudocs.models.Document` model:

    .. code-block:: python

      from tuiuiu.tuiuiudocs.models import Document
      from tuiuiu.tuiuiudocs.edit_handlers import DocumentChooserPanel
    ...
    As with images, Tuiuiu documents should also have the appropriate extra parameters to prevent cascade deletions across a ForeignKey relationship. ``DocumentChooserPanel`` takes only one argument: the name of the field.
...
.. versionchanged:: 1.1
    ...
    Before Tuiuiu 1.1, it was necessary to pass the snippet model class as a second parameter to ``SnippetChooserPanel``. This is now automatically picked up from the field.

.. module:: tuiuiu.tuiuiusnippets.edit_handlers

.. class:: SnippetChooserPanel(field_name, snippet_type=None)

    Snippets are vanilla Django models you create yourself without a Tuiuiu-provided base class. A chooser, ``SnippetChooserPanel``, is provided which takes the field name as an argument.

    .. code-block:: python

      from tuiuiu.tuiuiusnippets.edit_handlers import SnippetChooserPanel
...
Django's field types are automatically recognised and provided with an appropriate widget for input. Just define that field the normal Django way and pass the field name into :class:`~tuiuiu.tuiuiuadmin.edit_handlers.FieldPanel` when defining your panels. Tuiuiu will take care of the rest.

Here are some Tuiuiu-specific types that you might include as fields in your models.


Field Customisation
~~~~~~~~~~~~~~~~~~~

By adding CSS classes to your panel definitions or adding extra parameters to your field definitions, you can control much of how your fields will display in the Tuiuiu page editing interface. Tuiuiu's page editing interface takes much of its behaviour from Django's admin, so you may find many options for customisation covered there. (See `Django model field reference`_ ).
...
Use ``classname="full"`` to make a field (input element) stretch the full width of the Tuiuiu page editor. This will not work if the field is encapsulated in a :class:`~tuiuiu.tuiuiuadmin.edit_handlers.MultiFieldPanel`, which places its child fields into a formset.
...
Without a panel definition, a default form field (without label) will be used to represent your fields. If you intend to hide a field on the Tuiuiu page editor, define the field with ``editable=False`` (See `Django model field reference (editable)`_ ).
...
The ``django-modelcluster`` module allows for streamlined relation of extra models to a Tuiuiu page. For instance, you can create objects related through a ``ForeignKey`` relationship on the fly and save them to a draft revision of a ``Page`` object. Normally, your related objects "cluster" would need to be created beforehand (or asynchronously) before linking them to a Page.

Let's look at the example of adding related links to a :class:`~tuiuiu.tuiuiucore.models.Page`-derived model. We want to be able to add as many as we like, assign an order, and do all of this without leaving the page editing screen.

.. code-block:: python

  from tuiuiu.tuiuiucore.models import Orderable, Page
...
The ``RelatedLink`` class is a vanilla Django abstract model. The ``BookPageRelatedLinks`` model extends it with capability for being ordered in the Tuiuiu interface via the ``Orderable`` class as well as adding a ``page`` property which links the model to the ``BookPage`` model we're adding the related links objects to. Finally, in the panel definitions for ``BookPage``, we'll add an :class:`~tuiuiu.tuiuiuadmin.edit_handlers.InlinePanel` to provide an interface for it all. Let's look again at the parameters that :class:`~tuiuiu.tuiuiuadmin.edit_handlers.InlinePanel` accepts:

.. code-block:: python

    InlinePanel( relation_name, panels=None, label='', help_text='', min_num=None, max_num=None )

The ``relation_name`` is the ``related_name`` label given to the cluster's ``ParentalKey`` relation. You can add the ``panels`` manually or make them part of the cluster model. ``label`` and ``help_text`` provide a heading and caption, respectively, for the Tuiuiu editor. Finally, ``min_num`` and ``max_num`` allow you to set the minimum/maximum number of forms that the user must submit.

.. versionchanged:: 1.0

    In previous versions, it was necessary to pass the base model as the first parameter to :class:`~tuiuiu.tuiuiuadmin.edit_handlers.InlinePanel`; this is no longer required.
