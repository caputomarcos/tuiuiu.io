...
Tuiuiu supports Jinja2 templating for all front end features. More information on each of the template tags below can be found in the :ref:`writing_templates` documentation.
...
Django needs to be configured to support Jinja2 templates. As the Tuiuiu admin is written using regular Django templates, Django has to be configured to use both templating engines. Tuiuiu supports the Jinja2 backend that ships with Django 1.8 and above. Add the following configuration to the ``TEMPLATES`` setting for your app:
...
Transform Tuiuiu's internal HTML representation, expanding internal references to pages and images.
...
Output the Tuiuiu contextual flyout menu for editing pages from the front end

.. code-block:: html+jinja

    {{ tuiuiuuserbar() }}

See :ref:`tuiuiuuserbar_tag` for more information
