...
To do this, you need to add two models to your project:
 ...
 - The image model itself that inherits from
   ``tuiuiu.tuiuiuimages.models.AbstractImage``. This is where you would add
   your additional fields
 - The renditions model that inherits from
   ``tuiuiu.tuiuiuimages.models.AbstractRendition``. This is used to store
...
.. code-block:: python
    ...
    from tuiuiu.tuiuiuimages.models import Image, AbstractImage, AbstractRendition
...
.. versionchanged:: 1.10
    ...
    In previous versions of Tuiuiu it was necessary to connect signal handlers to handle deletion of image files. As of Tuiuiu 1.10 this is now handled automatically.
...
.. note::
    ...
    Tuiuiu needs to be able to create an image record immediately on upload.

Then set the ``TUIUIUIMAGES_IMAGE_MODEL`` setting to point to it:

.. code-block:: python

    TUIUIUIMAGES_IMAGE_MODEL = 'images.CustomImage'
