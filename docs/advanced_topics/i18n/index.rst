...
This document describes the internationalisation features of Tuiuiu and how to create multi-lingual sites.

Tuiuiu uses Django's `Internationalisation framework <https://docs.djangoproject.com/en/1.8/topics/i18n/>`_ so most of the steps are the same as other Django projects.


.. contents::


Tuiuiu admin translations
==========================

The Tuiuiu admin backend has been translated into many different languages. You can find a list of currently available translations on Tuiuiu's `Transifex page <https://www.transifex.com/torchbox/tuiuiu/>`_. (Note: if you're using an old version of Tuiuiu, this page may not accurately reflect what languages you have available).

If your language isn't listed on that page, you can easily contribute new languages or correct mistakes. Sign up and submit changes to `Transifex <https://www.transifex.com/torchbox/tuiuiu/>`_. Translation updates are typically merged into an official release within one month of being submitted.

Change Tuiuiu admin language on a per user basis
=================================================
.. versionadded:: 1.10


Logged-in users can set their preferred language from ``/admin/account/``.
By default, Tuiuiu provides a list of languages that have a >= 90% translation coverage.
It is possible to override this list via the :ref:`TUIUIUADMIN_PERMITTED_LANGUAGES <TUIUIUADMIN_PERMITTED_LANGUAGES>` setting.
...
Changing the primary language of your Tuiuiu installation
==========================================================

The default language of Tuiuiu is ``en-us`` (American English). You can change this by tweaking a couple of Django settings:
...
If there is a translation available for your language, the Tuiuiu admin backend should now be in the language you've chosen.
...
This section of the documentation will show you how to use Django's translation features with Tuiuiu and also describe a couple of methods for storing/retrieving translated content using Tuiuiu pages.
...
As Tuiuiu uses Django's templates, inserting this markup and the workflow for exporting and translating the strings is the same as any other Django project.
...
The most common approach for translating content in Tuiuiu is to duplicate each translatable text field, providing a separate field for each language.

This section will describe how to implement this method manually but there is a third party module you can use, `tuiuiu modeltranslation <https://github.com/infoportugal/tuiuiu-modeltranslation>`_, which may be quicker if it meets your needs.
...
.. code-block:: python
    ...
    class BlogPage(Page):
        ...
        thumbnail_image = models.ForeignKey('tuiuiuimages.image', ...)

.. note::

    We only define the French version of the ``title`` field as Tuiuiu already provides the English version for us.
