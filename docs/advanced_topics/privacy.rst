...
Private pages and documents work on Tuiuiu out of the box - the site implementer does not need to do anything to set them up. However, the default "log in" and "password required" forms are only bare-bones HTML pages, and site implementers may wish to replace them with a page customised to their site design.


Setting up a login page
~~~~~~~~~~~~~~~~~~~~~~~

The basic login page can be customised by setting ``TUIUIU_FRONTEND_LOGIN_TEMPLATE`` to the path of a template you wish to use:

.. code-block:: python

  TUIUIU_FRONTEND_LOGIN_TEMPLATE = 'myapp/login.html'

Tuiuiu uses Django's standard ``django.contrib.auth.views.login`` view here, and so the context variables available on the template are as detailed in `Django's login view documentation <https://docs.djangoproject.com/en/1.10/topics/auth/default/#django.contrib.auth.views.login>`_.

If the stock Django login view is not suitable - for example, you wish to use an external authentication system, or you are integrating Tuiuiu into an existing Django site that already has a working login view - you can specify the URL of the login view via the ``TUIUIU_FRONTEND_LOGIN_URL`` setting:

.. code-block:: python

  TUIUIU_FRONTEND_LOGIN_URL = '/accounts/login/'

To integrate Tuiuiu into a Django site with an existing login mechanism, setting ``TUIUIU_FRONTEND_LOGIN_URL = LOGIN_URL`` will usually be sufficient.
