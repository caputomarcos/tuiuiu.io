Your first Tuiuiu site
=======================

.. note::
   This tutorial covers setting up a brand new Tuiuiu project. If you'd like to add Tuiuiu to an existing Django project instead, see :doc:`integrating_into_django`.

1. Install Tuiuiu and its dependencies:
...
2. Start your site:
   ...
   Tuiuiu provides a ``start`` command similar to
   ``django-admin.py startproject``. Running ``tuiuiu start mysite`` in
   your project will generate a new ``mysite`` folder with a few
   Tuiuiu-specific extras, including the required project settings, a
...
Out of the box, the "home" app defines a blank ``HomePage`` model in ``models.py``, along with a migration that creates a homepage and configures Tuiuiu to use it.
...
``body`` is defined as ``RichTextField``, a special Tuiuiu field. You
...
You can now edit the homepage within the Tuiuiu admin area (go to Explorer, Homepage, then Edit) to see the new body field. Enter some text into the body field, and publish the page.

The page template now needs to be updated to reflect the changes made
to the model. Tuiuiu uses normal Django templates to render each page
...
Tuiuiu template tags
~~~~~~~~~~~~~~~~~~~~~

Tuiuiu provides a number of :ref:`template tags & filters <template-tags-and-filters>`
which can be loaded by including ``{% load tuiuiucore_tags %}`` at the top of
your template file.
...
**Note:** You'll need to include ``{% load tuiuiucore_tags %}`` in each
template that uses Tuiuiu's tags. Django will throw a ``TemplateSyntaxError``
...
``python manage.py startapp blog`` to create a new app in your Tuiuiu site.
...
takes a Tuiuiu Page object as an argument.

In the Tuiuiu admin, create a ``BlogIndexPage`` under the Homepage,
...
Note the use of Tuiuiu's built-in ``get_parent()`` method to obtain the
...
Tuiuiu gives you full control over what kinds of content can be created under
...
Much of the work you'll be doing in Tuiuiu revolves around the concept of hierarchical
...
Every "page" in Tuiuiu can call out to its parent or children
...
Tuiuiu provides the ``specific`` method that retrieves the actual ``BlogPage`` record.
...
When you start writing more customized Tuiuiu code, you'll find a whole set of QuerySet
...
QuerySet in the model definition. Tuiuiu makes this possible via
...
.. code-block:: python
    ...
    from tuiuiu.tuiuiucore.fields import RichTextField
    from tuiuiu.tuiuiuadmin.edit_handlers import FieldPanel, InlinePanel
    ...
    class BlogPageGalleryImage(Orderable):
        ...
        image = models.ForeignKey(
            'tuiuiuimages.Image', on_delete=models.CASCADE, related_name='+'
        )
...
the tagging system bundled with Tuiuiu, attach it to the ``BlogPage``
...
.. code-block:: python
    ...
    from tuiuiu.tuiuiucore.models import Page, Orderable
    from tuiuiu.tuiuiucore.fields import RichTextField
    from tuiuiu.tuiuiuadmin.edit_handlers import FieldPanel, InlinePanel, MultiFieldPanel
    from tuiuiu.tuiuiuimages.edit_handlers import ImageChooserPanel
    from tuiuiu.tuiuiusearch import index
...
Tuiuiu ecosystem, so that you can give it a title and URL in the
...
.. code-block:: python
    ...
    class BlogCategory(models.Model):
        ...
        icon = models.ForeignKey(
            'tuiuiuimages.Image', null=True, blank=True,
...
-  Read the Tuiuiu :doc:`topics <../topics/index>` and :doc:`reference <../reference/index>` documentation
