...
Tuiuiu API - A RESTful API for your Tuiuiu site
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

When installed, the new Tuiuiu API module provides a RESTful web API to your
Tuiuiu site. You can use this for accessing your raw field content for your
...
Tuiuiu now officially supports MySQL as a database backend.


Django 1.8 support
~~~~~~~~~~~~~~~~~~

Tuiuiu now officially supports running under Django 1.8.
...
Tuiuiu-specific additions. It includes bare minimum settings and two apps (home and search).
 ...
 * The external link chooser in rich text areas now accepts URLs of the form '/some/local/path', to allow linking to non-Tuiuiu-controlled URLs within the local site
 ...
 * SCSS files in tuiuiuadmin now use absolute imports, to permit overriding by user stylesheets
 * Removed the dependency on ``LOGIN_URL`` and ``LOGIN_REDIRECT_URL`` settings
 * Password reset view names namespaced to tuiuiuadmin
 * Removed the need to add permission check on admin views (now automated)
 * Reversing ``django.contrib.auth.admin.login`` will no longer lead to Tuiuius login view (making it easier to have frontend login views)
 * Added cache-control headers to all admin views. This allows Varnish/Squid/CDN to run on vanilla settings in front of a Tuiuiu site
 ...
 * Renamed the ``construct_tuiuiu_edit_bird`` hook to ``construct_tuiuiu_userbar``
 ...
 * Added :ref:`tuiuiusearch_backends_auto_update` flag to search backend settings to enable/disable automatically updating the search index on model changes
...
Tuiuiu version upgrade notifications are enabled by default
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Starting from Tuiuiu 1.0, the admin dashboard will (for admin users only) perform a check to see if newer releases are available. This also provides the Tuiuiu team with the hostname of your Tuiuiu site. If you’d rather not receive update notifications, or if you’d like your site to remain unknown, you can disable it by adding this line to your settings file:

.. code-block:: python

    TUIUIU_ENABLE_UPDATE_CHECK = False

``InlinePanel`` definitions no longer need to specify the base model
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In previous versions of Tuiuiu, inline child blocks on a page or snippet were defined using a declaration like:
...
As Tuiuiu uses Django's ``ModelForm`` for creating image model forms, we've added a new attribute called ``admin_form_fields``
that should be set to a ``tuple`` of field names on the image model.

See :ref:`custom_image_model` for an example.

You no longer need ``LOGIN_URL`` and ``LOGIN_REDIRECT_URL`` to point to Tuiuiu admin.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If you are upgrading from an older version of Tuiuiu, you probably want to remove these from your project settings.

Prevously, these two settings needed to be set to ``tuiuiuadmin_login`` and ``tuiuiuadmin_dashboard``
respectively or Tuiuiu would become very tricky to log in to. This is no longer the case and Tuiuiu
should work fine without them.

``RoutablePage`` now uses decorator syntax for defining views
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In previous versions of Tuiuiu, page types that used the :class:`~tuiuiu.contrib.tuiuiuroutablepage.models.RoutablePageMixin` had endpoints configured by setting their :attr:`~tuiuiu.contrib.tuiuiuroutablepage.models.RoutablePageMixin.subpage_urls` attribute to a list of urls with view names. This will not work on Django 1.8 as view names can no longer be passed into a url (see: https://docs.djangoproject.com/en/1.8/releases/1.8/#django-conf-urls-patterns).

Tuiuiu 1.0 introduces a new syntax where each view function is annotated with a ``@route`` decorator - see :ref:`routable_page_mixin`.
...
Upgrading from the external ``tuiuiuapi`` module.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If you were previously using the external ``tuiuiuapi`` module (which has now become ``tuiuiu.contrib.tuiuiuapi``). Please be aware of the following backwards-incompatible changes:
...
Previously, Tuiuiu would try to use Celery whenever the ``djcelery`` module was
...
It was previously possible to reverse the Tuiuiu login view using ``django.contrib.auth.views.login``.
This is no longer possible. Update any references to ``tuiuiuadmin_login``.

Password reset view name has changed from ``password_reset`` to ``tuiuiuadmin_password_reset``.

JavaScript includes in admin backend have been moved
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

To improve compatibility with third-party form widgets, pages within the Tuiuiu admin backend now output their JavaScript includes in the HTML header, rather than at the end of the page. If your project extends the admin backend (through the ``register_admin_menu_item`` hook, for example) you will need to ensure that all associated JavaScript code runs correctly from the new location. In particular, any code that accesses HTML elements will need to be contained in an 'onload' handler (e.g. jQuery's ``$(document).ready()``).

EditHandler internal API has changed
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

While it is not an official Tuiuiu API, it has been possible for Tuiuiu site implementers to define their own ``EditHandler`` subclasses for use in panel definitions, to customise the behaviour of the page / snippet editing forms. If you have made use of this facility, you will need to update your custom EditHandlers, as this mechanism has been refactored (to allow EditHandler classes to keep a persistent reference to their corresponding model). If you have only used Tuiuiu's built-in panel types (``FieldPanel``, ``InlinePanel``, ``PageChooserPanel`` and so on), you are unaffected by this change.

Previously, functions like ``FieldPanel`` acted as 'factory' functions, where a call such as ``FieldPanel('title')`` constructed and returned an ``EditHandler`` subclass tailored to work on a 'title' field. These functions now return an object with a ``bind_to_model`` method instead; the EditHandler subclass can be obtained by calling this with the model class as a parameter. As a guide to updating your custom EditHandler code, you may wish to refer to `the relevant change to the Tuiuiu codebase <https://github.com/tuiuiu/tuiuiu/commit/121c01c7f7db6087a985fa8dc9957bc78b9f6a6a>`_.

chooser_panel templates are obsolete
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If you have added your own custom admin views to the Tuiuiu admin (e.g. through the ``register_admin_urls`` hook), you may have used one of the following template includes to incorporate a chooser element for pages, documents, images or snippets into your forms:

- ``tuiuiuadmin/edit_handlers/chooser_panel.html``
- ``tuiuiuadmin/edit_handlers/page_chooser_panel.html``
- ``tuiuiudocs/edit_handlers/document_chooser_panel.html``
- ``tuiuiuimages/edit_handlers/image_chooser_panel.html``
- ``tuiuiusnippets/edit_handlers/snippet_chooser_panel.html``

All of these templates are now deprecated. Tuiuiu now provides a set of Django form widgets for this purpose - ``AdminPageChooser``, ``AdminDocumentChooser``, ``AdminImageChooser`` and ``AdminSnippetChooser`` - which can be used in place of the ``HiddenInput`` widget that these form fields were previously using. The field can then be rendered using the regular ``tuiuiuadmin/shared/field.html`` or ``tuiuiuadmin/shared/field_as_li.html`` template.
...
Previously, the forms for creating and editing images followed Django's default behaviour of showing all fields defined on the model; this would include any custom fields specific to your project that you defined by subclassing ``AbstractImage`` and setting ``TUIUIUIMAGES_IMAGE_MODEL``. This behaviour is risky as it may lead to fields being unintentionally exposed to the user, and so Django has deprecated this, for removal in Django 1.8. Accordingly, if you create your own custom subclass of ``AbstractImage``, you must now provide an ``admin_form_fields`` property, listing the fields that should appear on the image creation / editing form - for example:

.. code-block:: python

    from tuiuiu.tuiuiuimages.models import AbstractImage, Image
...
``construct_tuiuiu_edit_bird`` hook has been renamed
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Previously you could customize the Tuiuiu userbar using the ``construct_tuiuiu_edit_bird`` hook.
The hook has been renamed to ``construct_tuiuiu_userbar``.

The old hook is now deprecated; all existing ``construct_tuiuiu_edit_bird`` declarations should be updated to the new hook.


``IMAGE_COMPRESSION_QUALITY`` setting has been renamed
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The ``IMAGE_COMPRESSION_QUALITY`` setting, which determines the quality of saved JPEG images as a value from 1 to 100, has been renamed to ``TUIUIUIMAGES_JPEG_QUALITY``. If you have used this setting, please update your settings file accordingly.
