...
This work is the product of ``4`` Tuiuiu sprints, and the efforts of ``16`` people, listed here by order of first involvement:

 * Karl Hobley (Cape town sprint, admin API)
 * Josh Barr (Cape town sprint, prototype UI)
 * Thibaud Colas (Ede sprint, Reykjavík sprint)
 * Janneke Janssen (Ede sprint, Reykjavík sprint, Tuiuiu Space sprint)
 * Rob Moorman (Ede sprint, eslint-config-tuiuiu, ES6+React+Redux styleguide)
 ...
 * Hugo van den Berg (testing, Tuiuiu Space sprint)
 * Olly Willans (UI, UX, Tuiuiu Space sprint)
 * Andy Babic (UI, UX)
 * Ben Enright (UI, UX)
 ...
 * The lastmod field returned by tuiuiusitemaps now shows the last published date rather than the date of the last draft edit (Matt Westcott)
...
See also: :meth:`tuiuiu.tuiuiucore.models.Page.get_url_parts`, :meth:`tuiuiu.tuiuiucore.models.Page.get_url`,
:meth:`tuiuiu.tuiuiucore.models.Page.get_full_url`, and :meth:`tuiuiu.tuiuiucore.models.Page.relative_url`
